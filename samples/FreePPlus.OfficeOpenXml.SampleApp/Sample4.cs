...
class Sample4
{
    ...
    {
        Console.WriteLine("\nThis sample expects to connect to the AdventureWorks LT SQL database using the following connect string"
                          + $" - and the sample will fail if the database cannot be connected: '{connectionString}'");
        ...
        using (ExcelPackage p = new ExcelPackage(template, true))
        {
            ...
            {
                //The following SQL was written/tested with the AdventureWorksLT2022 SQL database from here:
                //  https://learn.microsoft.com/en-us/sql/samples/adventureworks-install-configure

                var sql = @"
declare @CurrencyRates table(
	CurrencyRateDate datetime
	,ToCurrencyCode varchar(3)
	,EndOfDayRate money
	,FromCurrencyCode varchar(3));

insert into @CurrencyRates(CurrencyRateDate, ToCurrencyCode, EndOfDayRate, FromCurrencyCode)
select 
	DateAdd(hh, soh.SalesOrderID * 2, soh.ShipDate) as CurrencyRateDate
	,'JPY' as ToCurrencyCode
	, ((soh.TaxAmt + soh.Freight) * 28) / soh.SubTotal as EndOfDayRate
...
select 
	DateAdd(hh, soh.SalesOrderID * 2, soh.ShipDate) as CurrencyRateDate
	,'EUR' as ToCurrencyCode
	, ((soh.TaxAmt + soh.Freight) * 8) / soh.SubTotal as EndOfDayRate
	,'USD' as FromCurrencyCode
	from SalesLT.SalesOrderHeader soh;
...
select 
	DateAdd(hh, soh.SalesOrderID * 2, soh.ShipDate) as CurrencyRateDate
	,'GBP' as ToCurrencyCode
	, ((soh.TaxAmt + soh.Freight) * 6) / soh.SubTotal as EndOfDayRate
	,'USD' as FromCurrencyCode
	from SalesLT.SalesOrderHeader soh;

SELECT 
	CurrencyRateDate
		,SUM(Case when ToCurrencyCode = 'JPY' Then EndOfDayRate Else 0 END) AS [JPY]
		,SUM(Case when ToCurrencyCode = 'EUR' Then EndOfDayRate Else 0 END) AS [EUR]
		,SUM(Case when ToCurrencyCode = 'GBP' Then EndOfDayRate Else 0 END) AS [GBP] 
	FROM @CurrencyRates  
	where [FromCurrencyCode]='USD' AND ToCurrencyCode in ('JPY', 'EUR', 'GBP') 
