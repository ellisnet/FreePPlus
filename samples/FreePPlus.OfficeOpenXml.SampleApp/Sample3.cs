        ...
        using (ExcelPackage xlPackage = new ExcelPackage(file))
        {
            ...
            if (worksheet != null)
            {
                ...
                using (SqlConnection sqlConn = new SqlConnection(connectionString))
                {
                    ...
                    var sql = @"
declare @Sales table(CustomerID int, SalesQuotaAvg money, SalesQuotaTotal money);

insert into @Sales(CustomerID, SalesQuotaAvg, SalesQuotaTotal)
	select CustomerID, AVG(TotalDue) / 6 as SalesQuotaAvg, SUM(TotalDue) as SalesQuotaTotal 
...
select distinct
		...
		, c.EmailAddress
		, a.City + ' Manager' as JobTitle
		, a.StateProvince + ', ' + a.CountryRegion as CountryRegionName
		, ISNULL(s.SalesQuotaAvg, 1000) AS SalesQuota
		, ISNULL(s.SalesQuotaAvg, 1000)*12 AS YearlyQuota
		, ISNULL(s.SalesQuotaTotal, 0) AS SalesYTD
	from [SalesLT].[Customer] c
		inner join [SalesLT].[CustomerAddress] ca on c.CustomerID = ca.CustomerID
		inner join [SalesLT].[Address] a on ca.AddressID = a.AddressID and ca.AddressType = 'Main Office'
	...
	ORDER BY SalesYTD desc;";
            ...
            xlPackage.Workbook.Properties.HyperlinkBase = new Uri("https://learn.microsoft.com/en-us/sql/samples/adventureworks-install-configure");
