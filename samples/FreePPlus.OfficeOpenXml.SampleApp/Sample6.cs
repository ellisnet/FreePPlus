...
class Sample6
{
    ...
    private static int AddDirectory(ExcelWorksheet ws, DirectoryInfo dir, int row, double height, int level, bool skipIcons)
    {
        ...
        if (!skipIcons)
        {
            Image icon = GetIcon(dir.FullName);
        ...
        foreach (FileInfo file in dir.GetFiles())
        {
            if (!skipIcons)
            {
                Image fileIcon = GetIcon(file.FullName);
    ...
    private static Image GetIcon(string fileName)
    {
        ...
        {
            result = Image.Load(Path.Combine(AppContext.BaseDirectory, "Resources", "folder_icon.png"));
        }
        else if (File.Exists(fileName))
        {
            result = Image.Load(Path.Combine(AppContext.BaseDirectory, "Resources", "file_icon.png"));
        }
