...
public abstract partial class Image
{
    ...
    private static IImageFormat InternalDetectFormat(Stream stream, Configuration config)
    {
        ...
        using (var buffer = config.MemoryAllocator.AllocateManagedByteBuffer(headerSize, AllocationOptions.Clean))
        {
...
#pragma warning restore SA1008 // Opening parenthesis must be spaced correctly
        where TPixel : unmanaged, IPixel<TPixel>
    {
        ...
        if (decoder is null) return (null, null);

        var img = decoder.Decode<TPixel>(config, stream);
        return (img, format);
    }
    ...
    private static (Image img, IImageFormat format) Decode(Stream stream, Configuration config)
    {
        ...
        var img = decoder.Decode(config, stream);
        return (img, format);
    }
    ...
    {
        if (!(DiscoverDecoder(stream, config, out var format) is IImageInfoDetector detector)) return (null, null);
