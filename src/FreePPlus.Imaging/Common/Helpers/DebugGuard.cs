...
// T-O-D-O: These should just call the guard equivalents
...
internal static partial class DebugGuard
{
    ...
    {
        if (!target) throw new InvalidOperationException(message);
    }
    ...
    public static void MustBeSameSized<T>(Span<T> target, Span<T> other, string parameterName)
        where T : struct
    {
        if (target.Length != other.Length) throw new ArgumentException("Span-s must be the same size!", parameterName);
    }
