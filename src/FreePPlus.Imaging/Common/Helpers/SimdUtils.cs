...
/// <summary>
///     Various extension and utility methods for <see cref="Vector4" /> and <see cref="Vector{T}" /> utilizing SIMD
///     capabilities
/// </summary>
internal static partial class SimdUtils
{
    ...
    internal static void ByteToNormalizedFloat(ReadOnlySpan<byte> source, Span<float> dest)
    {
        ...
        if (source.Length > 0) ConvertByteToNormalizedFloatRemainder(source, dest);
    }

    /// <summary>
    ///     Convert all <see cref="float" /> values normalized into [0..1] from 'source' into 'dest' buffer of
    ///     <see cref="byte" />.
    ...
    internal static void NormalizedFloatToByteSaturate(ReadOnlySpan<float> source, Span<byte> dest)
    {
        ...
        // Deal with the remainder:
        if (source.Length > 0) ConvertNormalizedFloatToByteRemainder(source, dest);
    }
    ...
    {
        return (byte)(f * 255f + 0.5f).Clamp(0, 255f);
    }

    [Conditional("DEBUG")]
    private static void VerifyHasVector8(string operation)
    {
        if (!HasVector8) throw new NotSupportedException($"{operation} is supported only on AVX2 CPU!");
    }
