...
public abstract partial class Image
{
    ...
    {
        return DetectFormat(Configuration.Default, stream);
    }
    ...
    {
        return WithSeekableStream(configuration, stream, s => InternalDetectFormat(s, configuration));
    }
    ...
    {
        return Identify(stream, out var _);
    }
    ...
    {
        return Identify(Configuration.Default, stream, out format);
    }
    ...
    {
        var data = WithSeekableStream(configuration, stream,
            s => InternalIdentity(s, configuration ?? Configuration.Default));
    ...
    {
        return Load(Configuration.Default, stream, out format);
    }
    ...
    {
        return Load(Configuration.Default, stream);
    }
    ...
    {
        return Load(Configuration.Default, stream, decoder);
    }
    ...
    {
        return Load(configuration, stream, out _);
    }
    ...
    public static Image<TPixel> Load<TPixel>(Stream stream, out IImageFormat format)
        where TPixel : unmanaged, IPixel<TPixel>
    {
        return Load<TPixel>(Configuration.Default, stream, out format);
    }
    ...
    public static Image<TPixel> Load<TPixel>(Stream stream, IImageDecoder decoder)
        where TPixel : unmanaged, IPixel<TPixel>
    {
        return WithSeekableStream(Configuration.Default, stream, s => decoder.Decode<TPixel>(Configuration.Default, s));
    }
    ...
    public static Image<TPixel> Load<TPixel>(Configuration configuration, Stream stream, IImageDecoder decoder)
        where TPixel : unmanaged, IPixel<TPixel>
    {
        return WithSeekableStream(configuration, stream, s => decoder.Decode<TPixel>(configuration, s));
    }
    ...
    public static Image<TPixel> Load<TPixel>(Configuration configuration, Stream stream)
        where TPixel : unmanaged, IPixel<TPixel>
    {
        return Load<TPixel>(configuration, stream, out var _);
    }
        ...
        where TPixel : unmanaged, IPixel<TPixel>
    {
        ...
        if (data.img != null) return data.img;
    ...
    {
        var data = WithSeekableStream(configuration, stream, s => Decode(s, configuration));
    ...
    private static T WithSeekableStream<T>(Configuration configuration, Stream stream, Func<Stream, T> action)
    {
        ...
        if (!stream.CanRead) throw new NotSupportedException("Cannot read from the stream.");

        if (stream.CanSeek)
        {
            if (configuration.ReadOrigin == ReadOrigin.Begin) stream.Position = 0;
