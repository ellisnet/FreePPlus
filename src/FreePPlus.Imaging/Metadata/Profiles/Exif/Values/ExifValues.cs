...
internal static class ExifValues
{
    ...
    {
        return (ExifValue)CreateValue(tag);
    }

    public static ExifValue Create(ExifTag tag)
    {
        return (ExifValue)CreateValue((ExifTagValue)(ushort)tag);
    }
    ...
    public static ExifValue Create(ExifTagValue tag, ExifDataType dataType, uint numberOfComponents)
    {
        ...
        switch (dataType)
        {
            case ExifDataType.Byte: return isArray ? new ExifByteArray(tag, dataType) : new ExifByte(tag, dataType);
            case ExifDataType.DoubleFloat: return isArray ? new ExifDoubleArray(tag) : new ExifDouble(tag);
            case ExifDataType.SingleFloat: return isArray ? new ExifFloatArray(tag) : new ExifFloat(tag);
            case ExifDataType.Long: return isArray ? new ExifLongArray(tag) : new ExifLong(tag);
            case ExifDataType.Rational: return isArray ? new ExifRationalArray(tag) : new ExifRational(tag);
            case ExifDataType.Short: return isArray ? new ExifShortArray(tag) : new ExifShort(tag);
            case ExifDataType.SignedByte: return isArray ? new ExifSignedByteArray(tag) : new ExifSignedByte(tag);
            case ExifDataType.SignedLong: return isArray ? new ExifSignedLongArray(tag) : new ExifSignedLong(tag);
            case ExifDataType.SignedRational:
                return isArray ? new ExifSignedRationalArray(tag) : new ExifSignedRational(tag);
            case ExifDataType.SignedShort: return isArray ? new ExifSignedShortArray(tag) : new ExifSignedShort(tag);
    ...
    private static object CreateValue(ExifTagValue tag)
    {
        switch (tag)
        {
            ...
            case ExifTagValue.GPSProcessingMethod:
                return new ExifByteArray(ExifTag.GPSProcessingMethod, ExifDataType.Undefined);
            case ExifTagValue.GPSAreaInformation:
                return new ExifByteArray(ExifTag.GPSAreaInformation, ExifDataType.Undefined);
