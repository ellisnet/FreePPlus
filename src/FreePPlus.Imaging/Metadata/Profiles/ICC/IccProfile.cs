...
public sealed class IccProfile : IDeepCloneable<IccProfile>
{
    ...
    public bool CheckIsValid()
    {
        ...
        var arrayValid = true;
        if (data != null)
            arrayValid = data.Length >= minSize &&
                         data.Length >= Header.Size;

        return arrayValid &&
               Enum.IsDefined(typeof(IccColorSpaceType), Header.DataColorSpace) &&
               Enum.IsDefined(typeof(IccColorSpaceType), Header.ProfileConnectionSpace) &&
               Enum.IsDefined(typeof(IccRenderingIntent), Header.RenderingIntent) &&
               Header.Size >= minSize &&
               Header.Size < maxSize;
    }
    ...
    private void InitializeEntries()
    {
        ...
        var reader = new IccReader();
        entries = reader.ReadTagData(data);
    }
}