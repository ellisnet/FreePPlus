...
public static class MemoryAllocatorExtensions
{
        ...
        where T : struct
    {
        ...
        var memoryGroup = memoryAllocator.AllocateGroup<T>(groupLength, width, options);
    ...
    public static Buffer2D<T> Allocate2D<T>(
        ...
        where T : struct
    {
        return Allocate2D<T>(memoryAllocator, size.Width, size.Height, options);
    }
    ...
    internal static MemoryGroup<T> AllocateGroup<T>(
        ...
        where T : struct
    {
        return MemoryGroup<T>.Allocate(memoryAllocator, totalLength, bufferAlignment, options);
    }
}