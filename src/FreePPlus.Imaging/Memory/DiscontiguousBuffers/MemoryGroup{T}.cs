    ...
    where T : struct
{
    ...
    IEnumerator<Memory<T>> IEnumerable<Memory<T>>.GetEnumerator()
    {
        ...
        throw new NotImplementedException(
            $"The type {GetType()} needs to override IEnumerable<Memory<T>>.GetEnumerator()");
    }
    ...
    /// <exception cref="InvalidMemoryOperationException">
    ///     Thrown when 'blockAlignment' converted to bytes is greater than the
    ///     buffer capacity of the allocator.
    /// </exception>
        ...
        AllocationOptions options = AllocationOptions.None)
    {
        ...
        var blockCapacityInElements = allocator.GetBufferCapacityInBytes() / ElementSize;
        ...
        var numberOfAlignedSegments = blockCapacityInElements / bufferAlignment;
        var bufferLength = numberOfAlignedSegments * bufferAlignment;
        if (totalLength > 0 && totalLength < bufferLength) bufferLength = (int)totalLength;

        var sizeOfLastBuffer = (int)(totalLength % bufferLength);
        var bufferCount = totalLength / bufferLength;
        ...
        for (var i = 0; i < buffers.Length - 1; i++) buffers[i] = allocator.Allocate<T>(bufferLength, options);

        if (bufferCount > 0) buffers[buffers.Length - 1] = allocator.Allocate<T>(sizeOfLastBuffer, options);
    ...
    public static MemoryGroup<T> Wrap(params Memory<T>[] source)
    {
        ...
        var totalLength = bufferLength > 0
            ? (long)bufferLength * (source.Length - 1) + source[source.Length - 1].Length
            : 0;
    ...
    public static MemoryGroup<T> Wrap(params IMemoryOwner<T>[] source)
    {
        ...
        var totalLength = bufferLength > 0
            ? (long)bufferLength * (source.Length - 1) + source[source.Length - 1].Memory.Length
            : 0;
