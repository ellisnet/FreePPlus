    ...
    where TPixel : unmanaged, IPixel<TPixel>
{
    ...
    /// <summary>
    ///     AHE tends to over amplify the contrast in near-constant regions of the image, since the histogram in such regions
    ///     is highly concentrated.
    ///     Clipping the histogram is meant to reduce this effect, by cutting of histogram bin's which exceed a certain amount
    ///     and redistribute
    ...
    public void ClipHistogram(Span<int> histogram, int clipLimit)
    {
        ...
        var addToEachBin = sumOverClip > 0 ? (int)MathF.Floor(sumOverClip / luminanceLevelsFloat) : 0;
        if (addToEachBin > 0)
            for (var i = 0; i < histogram.Length; i++)
                Unsafe.Add(ref histogramBase, i) += addToEachBin;

        var residual = sumOverClip - addToEachBin * LuminanceLevels;
        if (residual != 0)
        {
            var residualStep = Math.Max(LuminanceLevels / residual, 1);
            for (var i = 0; i < LuminanceLevels && residual > 0; i += residualStep, residual--)
            {
                ref var histogramLevel = ref Unsafe.Add(ref histogramBase, i);
                histogramLevel++;
            }
