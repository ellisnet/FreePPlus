...
public class HistogramEqualizationOptions
{
    ...
    ///     It is recommended to use clipping when the AdaptiveTileInterpolation method is used, to suppress artifacts which
    ///     can occur on the borders of the tiles.
    ///     Defaults to false.
    /// </summary>
    public bool ClipHistogram { get; set; } = false;

    /// <summary>
    ///     Gets or sets the histogram clip limit. Adaptive histogram equalization may cause noise to be amplified in near
    ///     constant
    ///     regions. To reduce this problem, histogram bins which exceed a given limit will be capped at this value. The
    ///     exceeding values
    ///     will be redistributed equally to all other bins. The clipLimit depends on the size of the tiles the image is split
    ///     into
    ...
    /// <summary>
    ///     Gets or sets the number of tiles the image is split into (horizontal and vertically) for the adaptive histogram
    ///     equalization.
    ///     Defaults to 8.
    /// </summary>
