...
{
    /// <summary>
    ///     The mapping of initialized complex kernels and parameters, to speed up the initialization of new
    ///     <see cref="BokehBlurProcessor{TPixel}" /> instances
    /// </summary>
    ...
    ///     The number of components to use to approximate the original 2D bokeh blur convolution
    ///     kernel.
    /// </param>
    ...
    private static void NormalizeKernels(Complex64[][] kernels, Vector4[] kernelParameters)
    {
        ...
        for (var i = 0; i < kernelsSpan.Length; i++)
        {
            ...
            for (var j = 0; j < length; j++) Unsafe.Add(ref valueRef, j) *= scalar;
        }
    }
}