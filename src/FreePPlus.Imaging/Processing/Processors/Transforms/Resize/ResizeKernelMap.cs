...
internal partial class ResizeKernelMap : IDisposable
{
        ...
        int radius)
    {
        ...
        data = memoryAllocator.Allocate2D<float>(MaxDiameter, bufferHeight, AllocationOptions.Clean);
        pinHandle = data.GetSingleMemory().Pin();
    ...
    internal virtual string Info =>
        $"radius:{radius}|sourceSize:{sourceLength}|destinationSize:{DestinationLength}|ratio:{ratio}|scale:{scale}";
    ...
    internal ref ResizeKernel GetKernel(int destIdx)
    {
        return ref kernels[destIdx];
    }
        ...
        where TResampler : struct, IResampler
    {
        ...
        var period = ImageMaths.LeastCommonMultiple(sourceSize, destinationSize) / sourceSize;
        ...
        // instead of Ceiling:
        if (TolerantMath.AreEqual(firstNonNegativeLeftVal, cornerInterval)) cornerInterval++;
    ...
    protected internal virtual void Initialize<TResampler>(in TResampler sampler)
        where TResampler : struct, IResampler
    {
        for (var i = 0; i < DestinationLength; i++) kernels[i] = BuildKernel(in sampler, i, i);
    }
    ...
    private ResizeKernel BuildKernel<TResampler>(in TResampler sampler, int destRowIndex, int dataRowIndex)
        where TResampler : struct, IResampler
    {
        var center = (destRowIndex + .5) * ratio - .5;

        // Keep inside bounds.
        var left = (int)TolerantMath.Ceiling(center - radius);
        if (left < 0) left = 0;

        var right = (int)TolerantMath.Floor(center + radius);
        if (right > sourceLength - 1) right = sourceLength - 1;

        var kernel = CreateKernel(dataRowIndex, left, right);

        var kernelValues = tempValues.AsSpan().Slice(0, kernel.Length);
        double sum = 0;

        for (var j = left; j <= right; j++)
        {
            double value = sampler.GetValue((float)((j - center) / scale));
            sum += value;
    ...
    private unsafe ResizeKernel CreateKernel(int dataRowIndex, int left, int right)
    {
        ...
        ValidateSizesForCreateKernel(length, dataRowIndex, left, right);

        var rowSpan = data.GetRowSpan(dataRowIndex);
