...
internal static class TransformUtilities
{
    ...
    {
        return IsNaN(matrix) || IsZero(matrix.GetDeterminant());
    }
    ...
    {
        return a > -Constants.EpsilonSquared && a < Constants.EpsilonSquared;
    }
    ...
    public static bool IsNaN(Matrix4x4 matrix)
    {
        return float.IsNaN(matrix.M11) || float.IsNaN(matrix.M12) || float.IsNaN(matrix.M13) || float.IsNaN(matrix.M14)
               || float.IsNaN(matrix.M21) || float.IsNaN(matrix.M22) || float.IsNaN(matrix.M23) ||
               float.IsNaN(matrix.M24)
               || float.IsNaN(matrix.M31) || float.IsNaN(matrix.M32) || float.IsNaN(matrix.M33) ||
               float.IsNaN(matrix.M34)
               || float.IsNaN(matrix.M41) || float.IsNaN(matrix.M42) || float.IsNaN(matrix.M43) ||
    ...
    {
        return CreateCenteredTransformMatrix(
    ...
    public static Matrix3x2 CreateCenteredTransformMatrix(Rectangle sourceRectangle, Matrix3x2 matrix)
    {
        ...
        var translationToTargetCenter =
            Matrix3x2.CreateTranslation(new Vector2(-destinationRectangle.Width, -destinationRectangle.Height) * .5F);
        var translateToSourceCenter =
            Matrix3x2.CreateTranslation(new Vector2(sourceRectangle.Width, sourceRectangle.Height) * .5F);
    ...
    ///     <see
    ///         href="https://docs.microsoft.com/en-us/xamarin/xamarin-forms/user-interface/graphics/skiasharp/transforms/non-affine" />
    /// </summary>
    ...
    {
        if (rectangle.Equals(default) || Matrix3x2.Identity.Equals(matrix)) return rectangle;
    ...
    public static Size GetTransformedSize(Size size, Matrix3x2 matrix)
    {
        ...
        if (matrix.Equals(default) || matrix.Equals(Matrix3x2.Identity)) return size;
    ...
    {
        if (rectangle.Equals(default) || Matrix4x4.Identity.Equals(matrix)) return rectangle;
    ...
    public static Size GetTransformedSize(Size size, Matrix4x4 matrix)
    {
        ...
        if (matrix.Equals(default) || matrix.Equals(Matrix4x4.Identity)) return size;
    ...
    private static Size ConstrainSize(Rectangle rectangle)
    {
        ...
        if (height <= 0) height = rectangle.Height;

        if (width <= 0) width = rectangle.Width;
