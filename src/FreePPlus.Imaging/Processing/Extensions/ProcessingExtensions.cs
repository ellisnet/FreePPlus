...
public static class ProcessingExtensions
{
    ...
    /// <exception cref="ObjectDisposedException">The source has been disposed.</exception>
    /// <exception cref="ImageProcessingException">The processing operation failed.</exception>
    public static void Mutate<TPixel>(this Image<TPixel> source, Configuration configuration,
    ...
    {
        return Clone(source, source.GetConfiguration(), operation);
    }
    ...
    /// <exception cref="ImageProcessingException">The processing operation failed.</exception>
    /// <returns>The new <see cref="Image{TPixel}" /></returns>
    public static Image<TPixel> Clone<TPixel>(this Image<TPixel> source, Configuration configuration,
        Action<IImageProcessingContext> operation)
        where TPixel : unmanaged, IPixel<TPixel>
    {
        Guard.NotNull(configuration, nameof(configuration));
    ...
    public static Image<TPixel> Clone<TPixel>(this Image<TPixel> source, params IImageProcessor[] operations)
        where TPixel : unmanaged, IPixel<TPixel>
    {
        return Clone(source, source.GetConfiguration(), operations);
    }
    ...
    public static IImageProcessingContext ApplyProcessors(
        ...
        params IImageProcessor[] operations)
    {
        foreach (var p in operations) source = source.ApplyProcessor(p);
    ...
    private class ProcessingVisitor : IImageVisitor
    {
        ...
        {
            var operationsRunner =
                configuration.ImageOperationsProvider.CreateImageProcessingContext(configuration, image, mutate);

            operation(operationsRunner);
            ResultImage = operationsRunner.GetResultImage();
        }
    }
}