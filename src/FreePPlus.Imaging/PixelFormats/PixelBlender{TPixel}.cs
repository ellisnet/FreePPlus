    ...
    where TPixel : unmanaged, IPixel<TPixel>
{
        ...
        where TPixelSrc : unmanaged, IPixel<TPixelSrc>
    {
               ...
               configuration.MemoryAllocator.Allocate<Vector4>(destination.Length * 3))
        {
            ...
            PixelOperations<TPixel>.Instance.ToVector4(configuration, sourcePixels, backgroundSpan,
                PixelConversionModifiers.Scale);
            var sourcePixels1 = source.Slice(0, background.Length);
            PixelOperations<TPixelSrc>.Instance.ToVector4(configuration, sourcePixels1, sourceSpan,
                PixelConversionModifiers.Scale);

            BlendFunction(destinationSpan, backgroundSpan, sourceSpan, amount);

            var sourceVectors = destinationSpan.Slice(0, background.Length);
            PixelOperations<TPixel>.Instance.FromVector4Destructive(configuration, sourceVectors, destination,
                PixelConversionModifiers.Scale);
        }
    }
    ...
    {
        Blend<TPixel>(configuration, destination, background, source, amount);
    }
