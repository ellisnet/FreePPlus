...
internal readonly struct PngHeader
{
    ...
    {
        if (!PngConstants.ColorTypes.TryGetValue(ColorType, out var supportedBitDepths))
        ...
        if (InterlaceMethod != PngInterlaceMode.None && InterlaceMethod != PngInterlaceMode.Adam7)
            throw new NotSupportedException(
                $"Invalid interlace method. Expected 'None' or 'Adam7'. Was '{InterlaceMethod}'.");
    }
    ...
    public static PngHeader Parse(ReadOnlySpan<byte> data)
    {
        return new PngHeader(
            BinaryPrimitives.ReadInt32BigEndian(data.Slice(0, 4)),
            BinaryPrimitives.ReadInt32BigEndian(data.Slice(4, 4)),
            data[8],
            (PngColorType)data[9],
            data[10],
            data[11],
            (PngInterlaceMode)data[12]);
    }
}