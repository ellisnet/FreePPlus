...
internal static class PngEncoderOptionsHelpers
{
    ...
    public static IndexedImageFrame<TPixel> CreateQuantizedFrame<TPixel>(
        ...
        where TPixel : unmanaged, IPixel<TPixel>
    {
        if (options.ColorType != PngColorType.Palette) return null;
        ...
        where TPixel : unmanaged, IPixel<TPixel>
    {
        ...
        {
            var quantizedBits = (byte)ImageMaths.GetBitsNeededForColorDepth(quantizedFrame.Palette.Length).Clamp(1, 8);
