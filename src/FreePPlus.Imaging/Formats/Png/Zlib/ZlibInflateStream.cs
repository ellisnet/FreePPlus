...
internal sealed class ZlibInflateStream : Stream
{
    ...
    public bool AllocateNewBytes(int bytes, bool isCriticalChunk)
    {
        ...
        if (CompressedStream is null) return InitializeInflateStream(isCriticalChunk);
    ...
    public override int Read(byte[] buffer, int offset, int count)
    {
        if (currentDataRemaining == 0)
        {
            ...
            if (currentDataRemaining == 0) return 0;
        }

        var bytesToRead = Math.Min(count, currentDataRemaining);
        currentDataRemaining -= bytesToRead;
        var totalBytesRead = innerStream.Read(buffer, offset, bytesToRead);
        ...
        while (currentDataRemaining == 0 && totalBytesRead < count)
        {
            currentDataRemaining = getData();

            if (currentDataRemaining == 0) return totalBytesRead;

            offset += bytesRead;

            if (offset >= innerStreamLength || offset >= count) return totalBytesRead;

            bytesToRead = Math.Min(count - totalBytesRead, currentDataRemaining);
            currentDataRemaining -= bytesToRead;
    ...
    private bool InitializeInflateStream(bool isCriticalChunk)
    {
        ...
        // |CMF|FLG|
        // +---+---+
        var cmf = innerStream.ReadByte();
        var flag = innerStream.ReadByte();
        currentDataRemaining -= 2;
        if (cmf == -1 || flag == -1) return false;
        ...
        if (fdict)
        {
            ...
            innerStream.Read(ChecksumBuffer, 0, 4);
            currentDataRemaining -= 4;
        }

        // Initialize the deflate Stream.
        CompressedStream = new DeflateStream(this, CompressionMode.Decompress, true);
