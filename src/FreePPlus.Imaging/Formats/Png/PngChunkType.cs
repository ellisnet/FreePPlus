...
internal enum PngChunkType : uint
{
    ...
    /// <summary>
    ///     Textual information that the encoder wishes to record with the image. The zTXt and tEXt chunks are semantically
    ///     equivalent,
    ///     but the zTXt chunk is recommended for storing large blocks of text. Each zTXt chunk contains a (uncompressed)
    ///     keyword and
    ...
    /// <summary>
    ///     The iTXt chunk contains International textual data. It contains a keyword, an optional language tag, an optional
    ///     translated keyword
    ...
    ///     the image samples conform to the colour space represented by the embedded ICC profile as defined by the
    ///     International Color Consortium.
    /// </summary>
    EmbeddedColorProfile = 0x69434350,

    /// <summary>
    ///     The sBIT chunk defines the original number of significant bits (which can be less than or equal to the sample
    ///     depth).
    ///     This allows PNG decoders to recover the original data losslessly even if the data had a sample depth not directly
    ///     supported by PNG.
    /// </summary>
    SignificantBits = 0x73424954,

    /// <summary>
    ///     If the sRGB chunk is present, the image samples conform to the sRGB colour space [IEC 61966-2-1] and should be
    ///     displayed
