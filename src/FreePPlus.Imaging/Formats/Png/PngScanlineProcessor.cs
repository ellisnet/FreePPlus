...
///     TODO: We should make this a stateful class or struct to reduce the number of arguments on methods (most are
///     invariant).
/// </summary>
internal static class PngScanlineProcessor
{
        ...
        where TPixel : unmanaged, IPixel<TPixel>
    {
        ...
        var scaleFactor = 255 / (ImageMaths.GetColorCountForBitDepth(header.BitDepth) - 1);
        ...
        if (!hasTrans)
        {
            if (header.BitDepth == 16)
            {
                ...
                for (int x = 0, o = 0; x < header.Width; x++, o += bytesPerPixel)
                {
                    ...
                    rgb48.G = BinaryPrimitives.ReadUInt16BigEndian(scanlineSpan.Slice(o + bytesPerSample,
                        bytesPerSample));
                    rgb48.B = BinaryPrimitives.ReadUInt16BigEndian(scanlineSpan.Slice(o + 2 * bytesPerSample,
                        bytesPerSample));
        ...
        where TPixel : unmanaged, IPixel<TPixel>
    {
        ...
        if (header.BitDepth == 16)
        {
            ...
            for (int x = 0, o = 0; x < header.Width; x++, o += bytesPerPixel)
            {
                ...
                rgba64.B = BinaryPrimitives.ReadUInt16BigEndian(scanlineSpan.Slice(o + 2 * bytesPerSample,
                    bytesPerSample));
                rgba64.A = BinaryPrimitives.ReadUInt16BigEndian(scanlineSpan.Slice(o + 3 * bytesPerSample,
                    bytesPerSample));
