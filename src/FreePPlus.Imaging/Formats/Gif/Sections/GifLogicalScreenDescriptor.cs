...
internal readonly struct GifLogicalScreenDescriptor
{
    ...
    /// </summary>
    public int GlobalColorTableSize => 2 << (Packed & 0x07);

    /// <summary>
    ///     Gets the color depth, in number of bits per pixel.
    ///     The lowest 3 packed bits represent the bit depth minus 1.
    /// </summary>
    public int BitsPerPixel => (Packed & 0x07) + 1;
    ...
    public static byte GetPackedValue(bool globalColorTableFlag, int colorResolution, bool sortFlag,
        int globalColorTableSize)
    {
        ...
        */
        ...
        if (globalColorTableFlag) value |= 1 << 7;

        value |= (byte)(colorResolution << 4);

        if (sortFlag) value |= 1 << 3;
