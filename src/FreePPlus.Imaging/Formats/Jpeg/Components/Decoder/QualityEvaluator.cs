...
///     Ported from
///     <see
///         href="https://github.com/ImageMagick/ImageMagick/blob/f362c02083d27211b913c6e44794f0ac6edaf2bd/coders/jpeg.c#L855" />
/// </summary>
internal static class QualityEvaluator
{
    ...
    public static int EstimateQuality(Block8x8F[] quantizationTables)
    {
        ...
        {
            ref var qTable = ref quantizationTables[i];
        ...
        ref var qTable0 = ref quantizationTables[0];
        ref var qTable1 = ref quantizationTables[1];
        ...
        if (!qTable0.Equals(default))
        {
            if (!qTable1.Equals(default))
            {
                ...
                for (var i = 0; i < 100; i++)
                {
                    if (quality < Hash[i] && sum < Sums[i]) continue;

                    if ((quality <= Hash[i] && sum <= Sums[i]) || i >= 50) return i + 1;
                }
            }
            else
            {
                ...
                for (var i = 0; i < 100; i++)
                {
                    if (quality < Hash1[i] && sum < Sums1[i]) continue;

                    if ((quality <= Hash1[i] && sum <= Sums1[i]) || i >= 50) return i + 1;
                }
            }
        }
