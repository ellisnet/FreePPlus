...
internal unsafe struct Block8x8 : IEquatable<Block8x8>
{
    ...
    public Block8x8(Span<short> coefficients)
    {
        ...
        ref var sourceRef = ref Unsafe.As<short, byte>(ref MemoryMarshal.GetReference(coefficients));
    ...
    {
        for (var i = 0; i < Size; i++) destination[i] = this[i];
    }
    ...
    {
        for (var i = 0; i < Size; i++) this[i] = (short)source[i];
    }
