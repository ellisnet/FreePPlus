...
internal static class JpegThrowHelper
{
    ...
    {
        throw new InvalidImageContentException($"Marker {marker} has bad length {length}.");
    }

    [MethodImpl(InliningOptions.ColdPath)]
    public static void ThrowBadQuantizationTable()
    {
        throw new InvalidImageContentException("Bad Quantization Table index.");
    }

    [MethodImpl(InliningOptions.ColdPath)]
    public static void ThrowBadSampling()
    {
        throw new InvalidImageContentException("Bad sampling factor.");
    }

    [MethodImpl(InliningOptions.ColdPath)]
    public static void ThrowBadProgressiveScan(int ss, int se, int ah, int al)
    {
        throw new InvalidImageContentException($"Invalid progressive parameters Ss={ss} Se={se} Ah={ah} Al={al}.");
    }

    [MethodImpl(InliningOptions.ColdPath)]
    public static void ThrowInvalidImageDimensions(int width, int height)
    {
        throw new InvalidImageContentException($"Invalid image dimensions: {width}x{height}.");
    }
}