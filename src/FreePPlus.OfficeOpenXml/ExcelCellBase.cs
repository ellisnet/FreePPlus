...
public abstract class ExcelCellBase
{
    ...
    private static string Translate(string value, dlgTransl addressTranslator, int row, int col)
    {
        ...
        foreach (var token in tokens)
            ...
            if (token.TokenType == TokenType.ExcelAddress || token.TokenType.Equals(TokenType.NameValue) ||
                token.TokenType == TokenType.ExcelAddressR1C1)
            {
    ...
    private static string ToR1C1(string part, int row, int col)
    {
        ...
        if (delim > 0)
        {
            var p1 = ToR1C1_1(part[..delim], row, col);
            var p2 = ToR1C1_1(part[(delim + 1)..], row, col);
            if (p1.Equals(p2))
                return p1;
            return sh + p1 + ":" + p2;
        }
    ...
    private static string ToR1C1_1(string part, int row, int col)
    {
        ...
        {
            if (addrRow == 0 && addrCol == 0) return part;
            if (addrRow > 0) sb.Append(fixRow ? $"R{addrRow}" : addrRow == row ? "R" : $"R[{addrRow - row}]");
            if (addrCol > 0) sb.Append(fixCol ? $"C{addrCol}" : addrCol == col ? "C" : $"C[{addrCol - col}]");
            return sb.ToString();
        }
    ...
    private static string ToAbs(string part, int row, int col)
    {
        ...
        if (delim > 0)
        {
            var p1 = ToAbs_1(part[..delim], row, col, false);
            var p2 = ToAbs_1(part[(delim + 1)..], row, col, false);
            if (p1.Equals(p2))
                return p1;
            return sh + p1 + ":" + p2;
        }
    ...
    {
        var check = ConvertUtil._invariantTextInfo.ToUpper(part);
        // Bug
        ...
        if (cStart == -1)
        {
            var RNum = GetRC(part[1..], row, out absoluteRow);
        ...
        if (rStart == -1)
        {
            var CNum = GetRC(part[1..], col, out absoluteCol);
    ...
    {
        int fromRow, fromCol;
        if (Address == "#REF!") return Address;
        if (GetRowCol(Address, out fromRow, out fromCol, false))
        {
            if (fromRow == 0 || fromCol == 0) return Address;
            if (rowIncr != 0 && row != 0 && fromRow >= row && Address.IndexOf('$', 1) == -1)
            {
                if (fromRow < row - rowIncr) return "#REF!";

                fromRow = fromRow + rowIncr;
            }

            if (colIncr != 0 && col != 0 && fromCol >= col &&
                ConvertUtil._invariantCompareInfo.IsPrefix(Address, "$") == false)
            {
                if (fromCol < col - colIncr) return "#REF!";

                fromCol = fromCol + colIncr;
            }

            Address = GetAddress(fromRow, Address.IndexOf('$', 1) > -1, fromCol,
    ...
    private static int GetRC(string value, int OffsetValue, out bool fixedAddr)
    {
        ...
        {
            fixedAddr = false;
            if (int.TryParse(value.Substring(1, value.Length - 2), NumberStyles.Any, CultureInfo.InvariantCulture,
                    out num))
    ...
    internal static bool GetRowColFromAddress(string CellAddress, out int FromRow, out int FromColumn, out int ToRow,
        out int ToColumn)
    {
        bool fixedFromRow, fixedFromColumn, fixedToRow, fixedToColumn;
        return GetRowColFromAddress(CellAddress, out FromRow, out FromColumn, out ToRow, out ToColumn, out fixedFromRow,
            out fixedFromColumn, out fixedToRow, out fixedToColumn);
    }
    ...
    /// <returns></returns>
    internal static bool GetRowColFromAddress(string CellAddress, out int FromRow, out int FromColumn, out int ToRow,
        out int ToColumn, out bool fixedFromRow, out bool fixedFromColumn, out bool fixedToRow, out bool fixedToColumn)
    {
        bool ret;
        ...
        if (CellAddress.IndexOf(' ') > 0) CellAddress = CellAddress[..CellAddress.IndexOf(' ')];
    ...
    internal static bool GetRowColFromAddress(string CellAddress, out int row, out int col, out bool fixedRow,
        out bool fixedCol)
    {
    ...
    internal static bool GetRowCol(string address, out int row, out int col, bool throwException, out bool fixedRow,
        out bool fixedCol)
    {
        var colPart = true;
        var colStartIx = 0;
        ...
        if (sheetMarkerIndex >= 0) colStartIx = sheetMarkerIndex + 1;
    ...
    private static int GetColumn(string sCol)
    {
        ...
        for (var i = len; i >= 0; i--) col += (sCol[i] - 64) * (int)Math.Pow(26, len - i);
        return col;
    }
    ...
    {
        if (FromRow == ToRow && FromColumn == ToColumn) return GetAddress(FromRow, FromColumn, Absolute);
    ...
    /// <returns>The cell address in the format A1</returns>
    public static string GetAddress(int FromRow, int FromColumn, int ToRow, int ToColumn, bool FixedFromRow,
        bool FixedFromColumn, bool FixedToRow, bool FixedToColumn)
    {
        ...
        if (address.IndexOf("!") == -1 || address == "#REF!")
        {
            if (fullRowCol)
            {
                ...
                if (cells.Length > 0)
                {
                    ...
                    if (cells.Length > 1) address += string.Format(":{0}", cells[1]);
                }
            }
            else
            {
                ...
                if ((a._fromRow == 1 && a._toRow == ExcelPackage.MaxRows) ||
                    ...
                    address = string.Format("'{0}'!{1}{2}:{3}{4}", worksheetName, GetColumnLetter(a._fromCol),
                        a._fromRow, GetColumnLetter(a._toCol), a._toRow);
                else
    ...
    internal static string UpdateFormulaReferences(string formula, int rowIncrement, int colIncrement, int afterRow,
        int afterColumn, string currentSheet, string modifiedSheet, bool setFixed = false)
    {
        ...
        try
        {
            ...
            foreach (var t in tokens)
                if (t.TokenType == TokenType.ExcelAddress)
                {
                    ...
                    var referencesModifiedWorksheet =
                        (string.IsNullOrEmpty(a._ws) &&
                         currentSheet.Equals(modifiedSheet, StringComparison.CurrentCultureIgnoreCase)) ||
                    ...
                    if (!string.IsNullOrEmpty(a._ws)) f += $"'{a._ws}'!";
                    if (rowIncrement > 0)
                        a = a.AddRow(afterRow, rowIncrement, setFixed);
                    else if (rowIncrement < 0) a = a.DeleteRow(afterRow, -rowIncrement, setFixed);
                    if (colIncrement > 0)
                        a = a.AddColumn(afterColumn, colIncrement, setFixed);
                    else if (colIncrement < 0) a = a.DeleteColumn(afterColumn, -colIncrement, setFixed);
