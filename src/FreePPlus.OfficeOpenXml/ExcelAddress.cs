...
{
    private static readonly HashSet<char> FormulaCharacters =
        new(new[] { '(', ')', '+', '-', '*', '/', '=', '^', '&', '%', '\"' });
    ...
    public ExcelCellAddress Start
    {
        get
        {
            if (_start == null) _start = new ExcelCellAddress(_fromRow, _fromCol);
            return _start;
        }
    }
    ...
    public ExcelCellAddress End
    {
        get
        {
            if (_end == null) _end = new ExcelCellAddress(_toRow, _toCol);
            return _end;
        }
    }
    ...
    private bool ExtractAddress(string fullAddress)
    {
        ...
        try
        {
                    ...
                    if (brackPos.Count > 0)
                    {
                        ...
                        else if (c == ']' && !isText)
                        {
                            if (brackPos.Count > 0)
                            {
                                ...
                                if (brackPos.Count == 0) HandleBrackets(first, second, bracketParts);
                            }
                            else
                            {
                                //Invalid address!
            ...
            if (Table == null) SetAddress(ref first, ref second, ref hasSheet, isMulti);
            return true;
        }
        catch
        {
    ...
    private void SetAddress(ref string first, ref string second, ref bool hasSheet, bool isMulti)
    {
        ...
        if (string.IsNullOrEmpty(_firstAddress))
        {
            ...
            GetRowColFromAddress(address, out _fromRow, out _fromCol, out _toRow, out _toCol, out _fromRowFixed,
                out _fromColFixed, out _toRowFixed, out _toColFixed);
        }
    ...
    {
        double d;
        if (Address == "#REF!") return AddressType.Invalid;

        if (double.TryParse(Address, NumberStyles.Any, CultureInfo.InvariantCulture,
                out d)) //A double, no valid address
            return AddressType.Invalid;

        if (IsFormula(Address)) return AddressType.Formula;

        if (r1c1 && IsR1C1(Address)) return AddressType.R1C1;
        ...
        if (SplitAddress(Address, out wb, out ws, out intAddress))
        {
            ...
            if (intAddress.Contains(",")) intAddress = intAddress[..intAddress.IndexOf(',')];
    ...
    private static bool IsR1C1(string address)
    {
        ...
        if (string.IsNullOrEmpty(address) || (address[0] != 'R' && address[0] != 'C')) return false;
    ...
    private static bool IsAddress(string intAddress)
    {
        ...
        if (!GetRowCol(cells[0], out fromRow, out fromCol, false)) return false;
        if (cells.Length > 1)
        {
            if (!GetRowCol(cells[1], out toRow, out toCol, false)) return false;
        }
        else
        {
            toRow = fromRow;
        ...
        for (var i = 0; i < address.Length; i++)
        {
            ...
            else
            {
                ...
                if (addressChar == '[' || addressChar == ']')
                    return false;
                if (isText == false && FormulaCharacters.Contains(addressChar)) return true;
            }
        }
    ...
    internal static string GetWorkbookPart(string address)
    {
        ...
        if (address[0] == '[')
        {
            ...
            if (ix > 0) return address.Substring(1, ix - 2);
        }
    ...
    internal static string GetWorksheetPart(string address, string defaultWorkSheet, ref int endIx)
    {
        ...
        var ix = 0;
        if (address[0] == '[') ix = address.IndexOf(']') + 1;
        if (ix > 0 && ix < address.Length)
        {
            if (address[ix] == '\'') return GetString(address, ix, out endIx);
    ...
    internal static void SplitAddress(string fullAddress, out string wb, out string ws, out string address,
        string defaultWorksheet = "")
    {
        ...
        {
            if (fullAddress[ix] == '!')
                address = fullAddress[(ix + 1)..];
            else
                address = fullAddress[ix..];
        }
        else
        {
            address = "";
    ...
    public ExcelAddressBase(int fromRow, int fromCol, int toRow, int toColumn, bool fromRowFixed, bool fromColFixed,
        bool toRowFixed, bool toColFixed)
    {
        _fromRow = fromRow;
        _toRow = toRow;
        _fromCol = fromCol;
        ...
        _address = GetAddress(_fromRow, _fromCol, _toRow, _toCol, _fromRowFixed, fromColFixed, _toRowFixed,
            _toColFixed);
    }
        ...
        if (string.IsNullOrEmpty(_wb) && Table != null)
            ...
            foreach (var t in ws.Tables)
                if (t.Name.Equals(Table.Name, StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    {
                        if (_fromCol <= 0 &&
                            cols[0].Equals(c.Name,
                                StringComparison.OrdinalIgnoreCase)) //Issue15063 Add invariant igore case
                        {
                            _fromCol = col;
                            if (cols.Length == 1)
                            {
                        ...
                        else if (cols.Length > 1 && _fromCol > 0 &&
                                 cols[1].Equals(c.Name,
                                     StringComparison.OrdinalIgnoreCase)) //Issue15063 Add invariant igore case
                        {
                            _toCol = col;
                            return;
    ...
    protected internal void SetAddress(string address)
    {
        ...
        if (ConvertUtil._invariantCompareInfo.IsPrefix(address, "'") ||
            ConvertUtil._invariantCompareInfo.IsPrefix(address, "["))
        ...
        {
            //Simple address
            GetRowColFromAddress(_address, out _fromRow, out _fromCol, out _toRow, out _toCol, out _fromRowFixed,
                out _fromColFixed, out _toRowFixed, out _toColFixed);
            _start = null;
            _end = null;
        }
    ...
    {
        var adr = "";
        if (!string.IsNullOrEmpty(_wb)) adr = "[" + _wb + "]";

        if (!string.IsNullOrEmpty(_ws)) adr += string.Format("'{0}'!", _ws);
        if (IsName)
            adr += GetAddress(_fromRow, _fromCol, _toRow, _toCol);
        else
            adr += GetAddress(_fromRow, _fromCol, _toRow, _toCol, _fromRowFixed, _fromColFixed, _toRowFixed,
                _toColFixed);
        return adr;
    }
    ...
    internal ExcelAddressBase AddRow(int row, int rows, bool setFixed = false)
    {
        if (row > _toRow)
            return this;
        if (row <= _fromRow)
            return new ExcelAddressBase(setFixed && _fromRowFixed ? _fromRow : _fromRow + rows, _fromCol,
                setFixed && _toRowFixed ? _toRow : _toRow + rows, _toCol, _fromRowFixed, _fromColFixed, _toRowFixed,
                _toColFixed);
        return new ExcelAddressBase(_fromRow, _fromCol, setFixed && _toRowFixed ? _toRow : _toRow + rows, _toCol,
            _fromRowFixed, _fromColFixed, _toRowFixed, _toColFixed);
    }
    ...
    internal ExcelAddressBase DeleteRow(int row, int rows, bool setFixed = false)
    {
        ...
        //Partly
        if (row <= _fromRow)
            return new ExcelAddressBase(row, _fromCol, setFixed && _toRowFixed ? _toRow : _toRow - rows, _toCol,
                _fromRowFixed, _fromColFixed, _toRowFixed, _toColFixed);
        return new ExcelAddressBase(_fromRow, _fromCol,
            setFixed && _toRowFixed ? _toRow : _toRow - rows < row ? row - 1 : _toRow - rows, _toCol, _fromRowFixed,
            _fromColFixed, _toRowFixed, _toColFixed);
    }
    ...
    internal ExcelAddressBase AddColumn(int col, int cols, bool setFixed = false)
    {
        if (col > _toCol)
            return this;
        if (col <= _fromCol)
            return new ExcelAddressBase(_fromRow, setFixed && _fromColFixed ? _fromCol : _fromCol + cols, _toRow,
                setFixed && _toColFixed ? _toCol : _toCol + cols, _fromRowFixed, _fromColFixed, _toRowFixed,
                _toColFixed);
        return new ExcelAddressBase(_fromRow, _fromCol, _toRow, setFixed && _toColFixed ? _toCol : _toCol + cols,
            _fromRowFixed, _fromColFixed, _toRowFixed, _toColFixed);
    }
    ...
    internal ExcelAddressBase DeleteColumn(int col, int cols, bool setFixed = false)
    {
        ...
        //Partly
        if (col <= _fromCol)
            return new ExcelAddressBase(_fromRow, col, _toRow, setFixed && _toColFixed ? _toCol : _toCol - cols,
                _fromRowFixed, _fromColFixed, _toRowFixed, _toColFixed);
        return new ExcelAddressBase(_fromRow, _fromCol, _toRow,
            setFixed && _toColFixed ? _toCol : _toCol - cols < col ? col - 1 : _toCol - cols, _fromRowFixed,
            _fromColFixed, _toRowFixed, _toColFixed);
    }

    internal ExcelAddressBase Insert(ExcelAddressBase address,
        eShiftType Shift /*, out ExcelAddressBase topAddress, out ExcelAddressBase leftAddress, out ExcelAddressBase rightAddress, out ExcelAddressBase bottomAddress*/)
    {
        ...
        if (Shift == eShiftType.Right)
        {
            if (address._fromRow > _fromRow)
                retAddress = GetAddress(_fromRow, _fromCol, address._fromRow, _toCol, _fromRowFixed, _fromColFixed,
                    _toRowFixed, _toColFixed);
            if (address._fromCol > _fromCol)
                retAddress = GetAddress(_fromRow < address._fromRow ? _fromRow : address._fromRow, _fromCol,
                    address._fromRow, _toCol, _fromRowFixed, _fromColFixed, _toRowFixed, _toColFixed);
        }
...
public class ExcelFormulaAddress : ExcelAddressBase
{
    ...
    public new string Address
    {
        ...
        {
            if (string.IsNullOrEmpty(_address) && _fromRow > 0)
                _address = GetAddress(_fromRow, _fromCol, _toRow, _toCol, _fromRowFixed, _toRowFixed, _fromColFixed,
                    _toColFixed);
            return _address;
        }
        set
    ...
    public new List<ExcelFormulaAddress> Addresses
    {
        get
        {
            if (_addresses == null) _addresses = new List<ExcelFormulaAddress>();
            return _addresses;
        }
    }
    ...
    internal string GetOffset(int row, int column)
    {
        ...
        if (!_fromRowFixed) fromRow += row;
        if (!_fromColFixed) fromCol += column;
        if (isMulti)
        {
            if (!_toRowFixed) toRow += row;
            if (!_toColFixed) tocol += column;
        }
        else
        {
            toRow = fromRow;
