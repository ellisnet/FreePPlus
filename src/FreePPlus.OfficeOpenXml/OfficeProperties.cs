...
public sealed class OfficeProperties : XmlHelper
{
        ...
        base(ns)
    {
        ...
        _coreHelper =
            XmlHelperFactory.Create(ns, CorePropertiesXml.SelectSingleNode("cp:coreProperties", NameSpaceManager));
    ...
    internal void Save()
    {
        if (_xmlPropertiesCore != null) _package.SavePart(_uriPropertiesCore, _xmlPropertiesCore);
        if (_xmlPropertiesExtended != null) _package.SavePart(_uriPropertiesExtended, _xmlPropertiesExtended);
        if (_xmlPropertiesCustom != null) _package.SavePart(_uriPropertiesCustom, _xmlPropertiesCustom);
    }
    ...
    public XmlDocument CorePropertiesXml
    {
        get
        {
            if (_xmlPropertiesCore == null)
            {
                ...
                _xmlPropertiesCore = GetXmlDocument(xml, _uriPropertiesCore,
    ...
    private XmlDocument GetXmlDocument(string startXml, Uri uri, string contentType, string relationship)
    {
        ...
        else
        {
            ...
            _package.Package.CreateRelationship(UriHelper.GetRelativeUri(new Uri("/xl", UriKind.Relative), uri),
    ...
    public XmlDocument ExtendedPropertiesXml
    {
        get
        {
            if (_xmlPropertiesExtended == null)
                _xmlPropertiesExtended = GetXmlDocument(string.Format(
    ...
    public string GetExtendedPropertyValue(string propertyName)
    {
        ...
        if (node != null) retValue = node.InnerText;
        return retValue;
    }
    ...
    public XmlDocument CustomPropertiesXml
    {
        get
        {
            if (_xmlPropertiesCustom == null)
                _xmlPropertiesCustom = GetXmlDocument(string.Format(
                        "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\" ?><Properties xmlns:vt=\"{0}\" xmlns=\"{1}\"></Properties>",
                        ExcelPackage.SchemaVt,
    ...
    public void SetCustomPropertyValue(string propertyName, object value)
    {
        ...
        if (node == null)
        {
            ...
            {
                if (!int.TryParse(MaxNode.Attributes["pid"].Value, out pid)) pid = 2;
                pid++;
            }
