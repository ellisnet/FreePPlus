...
public class SyntacticAnalyzer : ISyntacticAnalyzer
{
    public void Analyze(IEnumerable<Token> tokens)
    {
        ...
        {
            if (token.TokenType == TokenType.Unrecognized) throw new UnrecognizedTokenException(token);
    ...
    private static void Validate(AnalyzingContext context)
    {
        ...
        if (context.OpenedStrings != context.ClosedStrings) throw new FormatException("Unterminated string");
    }
    ...
    private void EnsureParenthesesAreWellFormed(Token token, AnalyzingContext context)
    {
        ...
        else if (token.TokenType == TokenType.ClosingParenthesis) context.NumberOfClosedParentheses++;
    }
