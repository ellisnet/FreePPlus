...
public class TokenFactory : ITokenFactory
{
    ...
    public TokenFactory(ITokenSeparatorProvider tokenSeparatorProvider, INameValueProvider nameValueProvider,
        IFunctionNameProvider functionNameProvider, bool r1c1)
    {
    ...
    public Token Create(IEnumerable<Token> tokens, string token, string worksheet)
    {
        ...
        if (_tokenSeparatorProvider.Tokens.TryGetValue(token, out tokenSeparator)) return tokenSeparator;
        ...
        if (Regex.IsMatch(token, RegexConstants.Decimal)) return new Token(token, TokenType.Decimal);
        if (Regex.IsMatch(token, RegexConstants.Integer)) return new Token(token, TokenType.Integer);
        ...
        if (token.ToUpper(CultureInfo.InvariantCulture) == "#NUM!") return new Token(token, TokenType.NumericError);
        if (token.ToUpper(CultureInfo.InvariantCulture) == "#VALUE!")
            return new Token(token, TokenType.ValueDataTypeError);
        if (token.ToUpper(CultureInfo.InvariantCulture) == "#NULL!") return new Token(token, TokenType.Null);
        if (_nameValueProvider != null && _nameValueProvider.IsNamedValue(token, worksheet))
            return new Token(token, TokenType.NameValue);
        if (_functionNameProvider.IsFunctionName(token)) return new Token(token, TokenType.Function);
