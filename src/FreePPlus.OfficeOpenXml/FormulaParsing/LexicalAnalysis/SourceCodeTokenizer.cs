...
public class SourceCodeTokenizer : ISourceCodeTokenizer
{
    ...
    public static ISourceCodeTokenizer R1C1 =>
        new SourceCodeTokenizer(FunctionNameProvider.Empty, NameValueProvider.Empty, true);

    public IEnumerable<Token> Tokenize(string input, string worksheet)
    {
        if (string.IsNullOrEmpty(input)) return Enumerable.Empty<Token>();
        ...
        while (handler.HasMore()) handler.Next();
        if (context.CurrentTokenHasValue) context.AddToken(CreateToken(context, worksheet));
    ...
    private static void CleanupTokens(TokenizerContext context, IDictionary<string, Token> tokens)
    {
        for (var i = 0; i < context.Result.Count; i++)
        {
            ...
            else if (token.TokenType == TokenType.WorksheetName)
            {
                ...
                {
                    for (var ix = 0; ix < 4; ix++) sb.Append(context.Result[i + ix].Value);
                }

                token.Value = sb.ToString();
                for (var ix = 0; ix < nToRemove; ix++) context.Result.RemoveAt(i + 1);
            }
                     ...
                     (token.Value == "+" || token.Value == "-"))
            {
                ...
                if (token.Value == "+" && (i == 0 || context.Result[i - 1].TokenType == TokenType.OpeningParenthesis ||
                                           context.Result[i - 1].TokenType == TokenType.Comma))
                {
    ...
    {
        if (context.Result[i].Value == "-" && i > 0 && (context.Result[i].TokenType == TokenType.Operator ||
