...
public class ExcelErrorCodes
{
    private static readonly IEnumerable<string> Codes = new List<string>
        { Value.Code, Name.Code, NoValueAvaliable.Code };
    ...
    {
        if (obj is ExcelErrorCodes) return ((ExcelErrorCodes)obj).Code.Equals(Code);
        return false;
    }
    ...
    public static bool IsErrorCode(object valueToTest)
    {
        ...
        if (Codes.FirstOrDefault(x => x == candidate) != null) return true;
        return false;
    }
}