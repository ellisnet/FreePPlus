...
public class ExpressionCompiler : IExpressionCompiler
{
    ...
    private CompileResult PerformCompilation(string worksheet = "", int row = -1, int column = -1)
    {
        ...
        if (_expressions.Any()) return compiledExpressions.First().Compile();
    ...
    private IEnumerable<Expression> HandleGroupedExpressions()
    {
        ...
        foreach (var groupedExpression in groupedExpressions)
        {
            ...
            if (groupedExpression.Prev != null) groupedExpression.Prev.Next = newExp;
            if (groupedExpression.Next != null) groupedExpression.Next.Prev = newExp;
            if (groupedExpression == first) first = newExp;
        }
    ...
    private IEnumerable<Expression> HandlePrecedenceLevel(int precedence)
    {
        ...
        do
        {
            ...
            if (compiledExpression is ExcelErrorExpression) return RefreshList(compiledExpression);
            if (expression == first) first = compiledExpression;
