...
public class ExpressionGraphBuilder : IExpressionGraphBuilder
{
    ...
    {
        if (token.TokenType == TokenType.String) return true;
        return false;
    }
    ...
    private void HandleFunctionArguments(Token[] tokens, Expression function)
    {
        ...
        if (token.TokenType != TokenType.OpeningParenthesis) throw new ExcelErrorValueException(eErrorType.Value);
        _tokenIndex++;
    ...
    private void SetOperatorOnExpression(Expression parent, IOperator op)
    {
        ...
        else
        {
            ...
            {
                candidate = parent.Children.Last();
                if (candidate is FunctionArgumentExpression) candidate = candidate.Children.Last();
            }
