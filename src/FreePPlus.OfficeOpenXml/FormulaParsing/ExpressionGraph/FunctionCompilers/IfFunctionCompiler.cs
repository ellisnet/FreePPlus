...
public class IfFunctionCompiler : FunctionCompiler
{
    ...
    public override CompileResult Compile(IEnumerable<Expression> children)
    {
        ...
        if (v is ExcelDataProvider.INameInfo) v = ((ExcelDataProvider.INameInfo)v).Value;

        if (v is ExcelDataProvider.IRangeInfo)
        {
            var r = (ExcelDataProvider.IRangeInfo)v;
            if (r.GetNCells() > 1) throw new ArgumentException("Logical can't be more than one cell");
            v = r.GetOffset(0, 0);
        }
