...
public class ValueMatcher
{
    ...
    public virtual int IsMatch(object o1, object o2)
    {
        ...
        if (o2.GetType() == typeof(string)) return CompareObjectToString(o1, o2.ToString());
    ...
    private static object CheckGetRange(object v)
    {
        if (v is ExcelDataProvider.IRangeInfo)
        {
            ...
            if (r.GetNCells() > 1) v = ExcelErrorValue.Create(eErrorType.NA);
            v = r.GetOffset(0, 0);
        }
    ...
    {
        double d1;
        if (double.TryParse(o1, out d1)) return d1.CompareTo(Convert.ToDouble(o2));
        bool b1;
        if (bool.TryParse(o1, out b1)) return b1.CompareTo(Convert.ToBoolean(o2));
        DateTime dt1;
        if (DateTime.TryParse(o1, out dt1)) return dt1.CompareTo(Convert.ToDateTime(o2));
    ...
    {
        double d2;
        if (double.TryParse(o2, out d2)) return Convert.ToDouble(o1).CompareTo(d2);
        return IncompatibleOperands;
    }
}