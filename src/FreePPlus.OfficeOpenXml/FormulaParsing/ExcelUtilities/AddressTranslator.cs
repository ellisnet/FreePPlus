...
public class AddressTranslator
{
    ...
    public virtual void ToColAndRow(string address, out int col, out int row, RangeCalculationBehaviour behaviour)
    {
        ...
        for (var x = 0; x < alphaPart.Length; x++)
        {
            ...
            if (pos == 0) col += currentNumericValue;
        }
    ...
    {
        if (behaviour == RangeCalculationBehaviour.FirstPart) return 1;
    ...
    {
        if (Regex.IsMatch(address, "[0-9]+")) return int.Parse(Regex.Match(address, "[0-9]+").Value);
        return null;
    }
}