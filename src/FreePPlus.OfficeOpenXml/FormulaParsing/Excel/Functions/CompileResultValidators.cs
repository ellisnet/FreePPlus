...
public class CompileResultValidators
{
    ...
    {
        if (_validators.ContainsKey(dataType)) return _validators[dataType];
        if (dataType == DataType.Decimal) return _validators[DataType.Decimal] = new DecimalCompileResultValidator();
