...
public class RowMatcher
{
    ...
    public bool IsMatch(ExcelDatabaseRow row, ExcelDatabaseCriteria criteria)
    {
        ...
        foreach (var c in criteria.Items)
        {
            ...
            {
                if (System.Math.Abs(ConvertUtil.GetValueDouble(candidate) - ConvertUtil.GetValueDouble(crit)) >
                    double.Epsilon) return false;
            }
            else
            {
                var criteriaString = crit.ToString();
                if (!Evaluate(candidate, criteriaString)) return false;
            }
        }
    ...
    private bool Evaluate(object obj, string expression)
    {
        ...
        if (ConvertUtil.IsNumeric(obj)) candidate = ConvertUtil.GetValueDouble(obj);
