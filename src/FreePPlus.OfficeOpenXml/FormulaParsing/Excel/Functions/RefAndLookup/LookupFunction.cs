...
public abstract class LookupFunction : ExcelFunction
{
    ...
    protected CompileResult Lookup(LookupNavigator navigator, LookupArguments lookupArgs)
    {
        ...
        if (lookupArgs.SearchedValue == null) return new CompileResult(eErrorType.NA);
        do
        {
            ...
            if (matchResult != 0)
            {
                ...
                if (lastValue == null && matchResult > 0) return new CompileResult(eErrorType.NA);
        ...
        return lookupArgs.RangeLookup
            ? _compileResultFactory.Create(lastLookupValue)
            : new CompileResult(eErrorType.NA);
    }
}