...
public class Match : LookupFunction
{
    ...
    public override CompileResult Execute(IEnumerable<FunctionArgument> arguments, ParsingContext context)
    {
        ...
        do
        {
            ...
            if (matchResult == 0) return CreateResult(navigator.Index + 1, DataType.Integer);

            if ((matchType == MatchType.ClosestBelow && matchResult < 0) ||
                (matchType == MatchType.ClosestAbove && matchResult > 0))
            ...
            else if (matchType == MatchType.ClosestBelow || matchType == MatchType.ClosestAbove) break;
    ...
    private MatchType GetMatchType(IEnumerable<FunctionArgument> arguments)
    {
        ...
        if (arguments.Count() > 2) matchType = (MatchType)ArgToInt(arguments, 2);
        return matchType;
    }
