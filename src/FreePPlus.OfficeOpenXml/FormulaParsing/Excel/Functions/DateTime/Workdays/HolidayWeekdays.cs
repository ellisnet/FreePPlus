...
public class HolidayWeekdays
{
    ...
    {
        foreach (var dayOfWeek in holidayDays) _holidayDays.Add(dayOfWeek);
    }
        ...
        if (holidays != null)
        {
            foreach (var arg in holidays)
                if (ConvertUtil.IsNumeric(arg.Value))
                {
                    ...
                    if (!IsHolidayWeekday(holidayDate)) resultDate = resultDate.AddDays(1);
                }
        }
        else
        {
    ...
    public System.DateTime GetNextWorkday(System.DateTime date,
        WorkdayCalculationDirection direction = WorkdayCalculationDirection.Forward)
    {
        ...
        while (IsHolidayWeekday(tmpDate)) tmpDate = tmpDate.AddDays(changeParam);
        return tmpDate;
    }
}