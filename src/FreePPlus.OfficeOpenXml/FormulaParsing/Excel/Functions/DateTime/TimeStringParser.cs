...
public class TimeStringParser
{
    ...
    {
        if (second < 0 || second > 59) throw new FormatException("Illegal value for second: " + second);
        if (minute < 0 || minute > 59) throw new FormatException("Illegal value for minute: " + minute);
    }
    ...
    {
        if (Regex.IsMatch(input, RegEx24)) return Parse24HourTimeString(input);
        if (Regex.IsMatch(input, RegEx12)) return Parse12HourTimeString(input);
    ...
    private static void GetValuesFromString(string input, out int hour, out int minute, out int second)
    {
        ...
        if (items.Length > 1) minute = int.Parse(items[1]);
        if (items.Length > 2)
        {
            var val = items[2];
