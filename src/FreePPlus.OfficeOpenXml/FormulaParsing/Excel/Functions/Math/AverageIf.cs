...
public class AverageIf : HiddenValuesHandlingFunction
{
    ...
    private bool Evaluate(object obj, string expression)
    {
        ...
        if (IsNumeric(obj)) candidate = ConvertUtil.GetValueDouble(obj);
        if (candidate.HasValue) return _expressionEvaluator.Evaluate(candidate.Value, expression);
    ...
    private double CalculateWithLookupRange(ExcelDataProvider.IRangeInfo range, string criteria,
        ExcelDataProvider.IRangeInfo sumRange, ParsingContext context)
    {
        var retVal = 0d;
        var nMatches = 0;
        foreach (var cell in range)
            if (criteria != null && Evaluate(cell.Value, criteria))
            {
                    ...
                    sumRange.Address._fromCol + oc <= sumRange.Address._toCol)
                {
                    ...
                    if (v is ExcelErrorValue) throw new ExcelErrorValueException((ExcelErrorValue)v);
                    nMatches++;
    ...
    private double CalculateSingleRange(ExcelDataProvider.IRangeInfo range, string expression, ParsingContext context)
    {
        ...
        foreach (var candidate in range)
            ...
            {
                if (candidate.IsExcelError) throw new ExcelErrorValueException((ExcelErrorValue)candidate.Value);
