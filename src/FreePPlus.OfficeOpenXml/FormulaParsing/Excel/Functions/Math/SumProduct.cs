...
public class SumProduct : ExcelFunction
{
    public override CompileResult Execute(IEnumerable<FunctionArgument> arguments, ParsingContext context)
    {
        ...
        foreach (var arg in arguments)
        {
            ...
            {
                foreach (var val in (IEnumerable<FunctionArgument>)arg.Value) AddValue(val.Value, currentResult);
            }
        ...
        {
            double rowResult = 1;
            for (var colIndex = 0; colIndex < results.Count; colIndex++) rowResult *= results[colIndex][rowIndex];
            result += rowResult;
        }
