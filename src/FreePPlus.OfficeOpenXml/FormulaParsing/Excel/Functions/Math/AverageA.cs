...
public class AverageA : HiddenValuesHandlingFunction
{
    public override CompileResult Execute(IEnumerable<FunctionArgument> arguments, ParsingContext context)
    {
        ...
        foreach (var arg in arguments) Calculate(arg, context, ref result, ref nValues);
    ...
    private double? GetNumericValue(object obj, bool isInArray)
    {
        ...
        if (IsNumeric(obj) && !(obj is bool)) return ConvertUtil.GetValueDouble(obj);
        if (!isInArray)
        {
            if (obj is bool)
            {
            ...
            if (ConvertUtil.TryParseNumericString(obj, out number))
                return number;
            if (ConvertUtil.TryParseDateString(obj, out date)) return date.ToOADate();
        }
