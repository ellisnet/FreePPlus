...
public class CountA : HiddenValuesHandlingFunction
{
        ...
        foreach (var item in items)
        {
            ...
            if (cs != null)
            {
                foreach (var c in cs)
                {
                    ...
                    if (!ShouldIgnore(c, context) && ShouldCount(c.Value)) nItems++;
                }
            }
            ...
            else
            {
                ...
                if (!ShouldIgnore(item) && ShouldCount(item.Value)) nItems++;
            }
        }
    }
    ...
    {
        if (context.Scopes.Current.IsSubtotal) CheckForAndHandleExcelError(arg);
    }

    private void _CheckForAndHandleExcelError(ExcelDataProvider.ICellInfo cell, ParsingContext context)
    {
        if (context.Scopes.Current.IsSubtotal) CheckForAndHandleExcelError(cell);
    }
