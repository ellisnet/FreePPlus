...
public class FunctionRepository : IFunctionNameProvider
{
    ...
    /// </summary>
    /// <param name="module">
    ///     A <see cref="IFunctionModule" /> that can be used for adding functions and custom function
    ///     compilers.
    /// </param>
    public virtual void LoadModule(IFunctionModule module)
    {
        ...
        foreach (var key in module.CustomCompilers.Keys) CustomCompilers[key] = module.CustomCompilers[key];
    }
    ...
    public void AddOrReplaceFunction(string functionName, ExcelFunction functionImpl)
    {
        ...
        if (_functions.ContainsKey(fName)) _functions.Remove(fName);
