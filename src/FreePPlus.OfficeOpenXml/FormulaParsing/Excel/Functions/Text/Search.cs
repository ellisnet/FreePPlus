...
public class Search : ExcelFunction
{
    public override CompileResult Execute(IEnumerable<FunctionArgument> arguments, ParsingContext context)
    {
        ...
        var startIndex = 0;
        if (functionArguments.Count() > 2) startIndex = ArgToInt(functionArguments, 2);
        var result = searchIn.IndexOf(search, startIndex, StringComparison.OrdinalIgnoreCase);
        if (result == -1) return CreateResult(ExcelErrorValue.Create(eErrorType.Value), DataType.ExcelError);
