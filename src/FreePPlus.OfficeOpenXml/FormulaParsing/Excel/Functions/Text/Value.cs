...
public class Value : ExcelFunction
{
    ...
    public override CompileResult Execute(IEnumerable<FunctionArgument> arguments, ParsingContext context)
    {
        ...
        if (Regex.IsMatch(val,
                $"^[\\d]*({Regex.Escape(_groupSeparator)}?[\\d]*)?({Regex.Escape(_decimalSeparator)}[\\d]*)*?[ ?% ?]?$"))
        {
            if (val.EndsWith("%"))
            {
                val = val.TrimEnd('%');
        ...
        if (Regex.IsMatch(val, @"^[\d]{1,2}" + timeSeparator + @"[\d]{2}(" + timeSeparator + @"[\d]{2})?$"))
        {
            ...
            if (timeResult.DataType == DataType.Date) return timeResult;
        }

        var dateResult = _dateValueFunc.Execute(val);
        if (dateResult.DataType == DataType.Date) return dateResult;
