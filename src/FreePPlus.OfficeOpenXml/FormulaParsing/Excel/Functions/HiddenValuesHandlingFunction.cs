...
public abstract class HiddenValuesHandlingFunction : ExcelFunction
{
    ...
    protected override IEnumerable<ExcelDoubleCellValue> ArgsToDoubleEnumerable(IEnumerable<FunctionArgument> arguments,
    ...
    protected IEnumerable<ExcelDoubleCellValue> ArgsToDoubleEnumerable(IEnumerable<FunctionArgument> arguments,
        ParsingContext context, bool ignoreErrors)
    {
        if (!arguments.Any()) return Enumerable.Empty<ExcelDoubleCellValue>();
    ...
    {
        if (IgnoreHiddenValues && arg.ExcelStateFlagIsSet(ExcelCellState.HiddenCell)) return true;
        return false;
    }
}