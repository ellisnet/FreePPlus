...
public abstract class ExcelFunction
{
    ...
    ///     Arguments to the function, each argument can contain primitive types, lists or
    ///     <see cref="ExcelDataProvider.IRangeInfo">Excel ranges</see>
    /// </param>
    ...
    /// <param name="errorTypeToThrow">
    ///     The <see cref="eErrorType" /> of the <see cref="ExcelErrorValueException" /> that will
    ///     be thrown if <paramref name="minLength" /> is not met.
    /// </param>
    ...
    {
        return arguments.ElementAt(index).IsExcelRange
            ? arguments.ElementAt(index).ValueAsRangeInfo.Address.FullAddress
            : ArgToString(arguments, index);
    }
    ...
    protected string ArgToAddress(IEnumerable<FunctionArgument> arguments, int index, ParsingContext context)
    {
        ...
        if (arg.ExcelAddressReferenceId > 0) return context.AddressCache.Get(arg.ExcelAddressReferenceId);
    ...
    {
        if (System.Math.Abs(right - 0d) < double.Epsilon) throw new ExcelErrorValueException(eErrorType.Div0);
        return left / right;
    }
    ...
    protected bool IsNumeric(object val)
    {
        ...
        return TypeCompat.IsPrimitive(val) || val is double || val is decimal || val is System.DateTime ||
               val is TimeSpan;
    }
    ...
    protected virtual IEnumerable<ExcelDoubleCellValue> ArgsToDoubleEnumerable(bool ignoreHiddenCells,
        bool ignoreErrors, IEnumerable<FunctionArgument> arguments, ParsingContext context)
    {
    ...
    protected virtual IEnumerable<double> ArgsToDoubleEnumerableZeroPadded(bool ignoreHiddenCells,
        ExcelDataProvider.IRangeInfo rangeInfo, ParsingContext context)
    {
    ...
    /// <param name="context"></param>
    /// <returns></returns>
    protected virtual IEnumerable<object> ArgsToObjectEnumerable(bool ignoreHiddenCells,
        IEnumerable<FunctionArgument> arguments, ParsingContext context)
    {
    ...
    /// <returns></returns>
    protected virtual double CalculateCollection(IEnumerable<FunctionArgument> collection, double result,
    ...
    {
        if (IsNumeric(result)) return CreateResult(result, DataType.Decimal);
        if (result is string) return CreateResult(result, DataType.String);
        if (ExcelErrorValue.Values.IsErrorValue(result)) return CreateResult(result, DataType.ExcelAddress);
        if (result == null) return CompileResult.Empty;
