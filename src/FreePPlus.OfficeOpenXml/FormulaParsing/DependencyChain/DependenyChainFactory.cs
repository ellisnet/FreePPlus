...
internal static class DependencyChainFactory
{
    ...
    private static void GetChain(DependencyChain depChain, ILexer lexer, ExcelWorksheet ws, string formula,
        ExcelCalculationOption options)
    {
        ...
        ExcelCalculationOption options)
    {
        ...
        var fs = new CellsStoreEnumerator<object>(ws._formulas, Range.Start.Row, Range.Start.Column, Range.End.Row,
            Range.End.Column);
        while (fs.Next())
        {
    ...
    ///     Goto (!) is used internally to prevent stackoverflow on extremly larget dependency trees (that is, many recursive
    ///     formulas).
    /// </summary>
    ...
    private static void FollowChain(DependencyChain depChain, ILexer lexer, ExcelWorkbook wb, ExcelWorksheet ws,
        FormulaCell f, ExcelCalculationOption options)
    {
        ...
        while (f.tokenIx < f.Tokens.Count)
        {
            ...
            if (t.TokenType == TokenType.ExcelAddress)
            {
                ...
                if (adr.WorkSheet == null &&
                    adr.Collide(new ExcelAddressBase(f.Row, f.Column, f.Row, f.Column)) !=
                    ExcelAddressBase.eAddressCollition.No && !options.AllowCirculareReferences)
                    throw new CircularReferenceException(string.Format("Circular Reference in cell {0}",
                ...
                if (adr._fromRow > 0 && adr._fromCol > 0)
                {
                    if (string.IsNullOrEmpty(adr.WorkSheet))
                    {
                        if (f.ws == null)
                            f.ws = ws;
                        else if (f.ws.SheetID != f.SheetID) f.ws = wb.Worksheets.GetBySheetID(f.SheetID);
                    }
                    else
                    {
                    ...
                    if (f.ws != null)
                    {
                        f.iterator = new CellsStoreEnumerator<object>(f.ws._formulas, adr.Start.Row, adr.Start.Column,
                            adr.End.Row, adr.End.Column);
                        goto iterateCells;
                    }
                }
            }
            else if (t.TokenType == TokenType.NameValue)
            {
                ...
                ExcelAddressBase.SplitAddress(t.Value, out adrWb, out adrWs, out adrName,
                    f.ws == null ? "" : f.ws.Name);
                if (!string.IsNullOrEmpty(adrWs))
                {
                    if (f.ws == null) f.ws = wb.Worksheets[adrWs];
                ...
                else if (wb.Names.ContainsKey(adrName))
                {
                    ...
                    if (string.IsNullOrEmpty(adrWs)) f.ws = name.Worksheet;
                }
                else
                {
                    name = null;
                ...
                if (name != null)
                {
                    ...
                    else
                    {
                        ...
                        if (!depChain.index.ContainsKey(id))
                        {
                            ...
                            rf.Tokens = name.LocalSheetId == -1
                                ? lexer.Tokenize(rf.Formula).ToList()
                                : lexer.Tokenize(rf.Formula, wb.Worksheets.GetBySheetID(name.LocalSheetId).Name)
                                    .ToList();
                        ...
                        if (stack.Count > 0)
                            ...
                            foreach (var par in stack)
                                if (ExcelCellBase.GetCellID(par.SheetID, par.Row, par.Column) == id &&
                                    !options.AllowCirculareReferences)
                                    throw new CircularReferenceException(string.Format("Circular Reference in name {0}",
                                        name.Name));
                    }
                }
            }
                    ...
                    if (ExcelCellBase.GetCellID(par.ws.SheetID, par.iterator.Row, par.iterator.Column) == id)
                    {
                        if (options.AllowCirculareReferences == false)
                        {
                            throw new CircularReferenceException(string.Format("Circular Reference in cell {0}!{1}",
                                par.ws.Name, ExcelCellBase.GetAddress(f.Row, f.Column)));
                        }
