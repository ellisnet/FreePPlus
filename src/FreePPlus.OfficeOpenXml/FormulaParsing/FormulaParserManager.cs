...
public class FormulaParserManager
{
    ...
    ///     If it exists, the existing function will be replaced by the supplied
    ///     <paramref name="functionImpl">function implementation</paramref>
    /// </summary>
    ...
    public void CopyFunctionsFrom(ExcelWorkbook otherWorkbook)
    {
        ...
        foreach (var func in functions) AddOrReplaceFunction(func.Key, func.Value);
    }
    ...
    /// <summary>
    ///     Returns an enumeration of all implemented functions, including the implementing <see cref="ExcelFunction" />
    ///     instance.
    /// </summary>
    ...
    public IEnumerable<KeyValuePair<string, ExcelFunction>> GetImplementedFunctions()
    {
        ...
        _parser.Configure(parsingConfiguration =>
        {
            foreach (var name in parsingConfiguration.FunctionRepository.FunctionNames)
                functions.Add(new KeyValuePair<string, ExcelFunction>(name,
                    parsingConfiguration.FunctionRepository.GetFunction(name)));
        });
        return functions;
    }
