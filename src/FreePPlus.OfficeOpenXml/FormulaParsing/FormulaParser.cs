...
public class FormulaParser : IDisposable
{
    ...
    public FormulaParser(ExcelDataProvider excelDataProvider, ParsingContext parsingContext)
    {
        ...
        Configure(configuration =>
        {
            configuration
                .SetLexer(
                    new Lexer(_parsingContext.Configuration.FunctionRepository, _parsingContext.NameValueProvider))
    ...
    public void Dispose()
    {
        if (_parsingContext.Debug) _parsingContext.Configuration.Logger.Dispose();
    }
    ...
    internal virtual object Parse(string formula, RangeAddress rangeAddress)
    {
        using (var scope = _parsingContext.Scopes.NewScope(rangeAddress))
        {
            ...
            if (graph.Expressions.Count() == 0) return null;
    ...
    internal virtual object ParseCell(IEnumerable<Token> tokens, string worksheet, int row, int column)
    {
        ...
        using (var scope = _parsingContext.Scopes.NewScope(rangeAddress))
        {
            ...
            var graph = _graphBuilder.Build(tokens);
            if (graph.Expressions.Count() == 0) return 0d;
            try
            {
                ...
                if (rangeInfo.IsEmpty) return 0d;
                if (!rangeInfo.IsMulti) return rangeInfo.First().Value ?? 0d;
                // ok to return multicell if it is a workbook scoped name.
                if (string.IsNullOrEmpty(worksheet)) return rangeInfo;
            ...
            {
                if (_parsingContext.Debug) _parsingContext.Configuration.Logger.Log(_parsingContext, ex);
    ...
    {
        if (_excelDataProvider != null) _excelDataProvider.Reset();
    }
}