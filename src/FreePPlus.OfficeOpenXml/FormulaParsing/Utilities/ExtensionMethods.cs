...
public static class ExtensionMethods
{
    ...
    {
        if (string.IsNullOrEmpty(val.Value)) throw new ArgumentException(val.Name + " cannot be null or empty");
    }

    public static void IsNotNull<T>(this ArgumentInfo<T> val)
        where T : class
    {
        if (val.Value == null) throw new ArgumentNullException(val.Name);
    }
