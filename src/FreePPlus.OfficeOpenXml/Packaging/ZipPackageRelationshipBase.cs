...
public abstract class ZipPackageRelationshipBase
{
    ...
    protected void UpdateMaxRId(string id, ref int maxRId)
    {
        ...
        {
            int num;
            if (int.TryParse(id[3..], out num))
                if (num == maxRId - 1)
                    maxRId--;
        }
    }

    internal virtual ZipPackageRelationship CreateRelationship(Uri targetUri, TargetMode targetMode,
    ...
    {
        return relationships.GetRelationshipsByType(schema);
    }
    ...
    internal void ReadRelation(string xml, string source)
    {
        ...
        if (doc.DocumentElement != null)
            foreach (XmlElement c in doc.DocumentElement.ChildNodes)
            {
                ...
                rel.TargetMode = c.GetAttribute("TargetMode").Equals("external", StringComparison.OrdinalIgnoreCase)
                    ? TargetMode.External
                    : TargetMode.Internal;
                try
                {
                ...
                catch
                {
                    ...
                    rel.TargetUri = new Uri(Uri.EscapeDataString("Invalid:URI " + c.GetAttribute("Target")),
                ...
                {
                    int id;
                    if (int.TryParse(rel.Id[3..], out id))
                        if (id >= maxRId &&
                            id < int.MaxValue -
    ...
    protected ZipPackageRelationshipCollection relationships = new();
