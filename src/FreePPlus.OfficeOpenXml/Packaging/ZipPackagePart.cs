...
internal class ZipPackagePart : ZipPackageRelationshipBase, IDisposable
{
        ...
        internal set
        {
            if (!string.IsNullOrEmpty(_contentType))
                if (Package._contentTypes.ContainsKey(Package.GetUriKey(Uri.OriginalString)))
                {
                    ...
                    Package._contentTypes.Add(Package.GetUriKey(Uri.OriginalString),
                        new ZipPackage.ContentType(value, false, Uri.OriginalString));
                }
    ...
    internal override ZipPackageRelationship CreateRelationship(Uri targetUri, TargetMode targetMode,
    ...
    internal void WriteZip(ZipOutputStream os)
    {
        ...
        if (relationships.Count > 0)
        {
            ...
            relationships.WriteZip(os, string.Format("{0}_rels/{1}.rels", f[..^name.Length], name));
        }
    }

    internal delegate void SaveHandlerDelegate(ZipOutputStream stream, CompressionLevel compressionLevel,
        string fileName);
}