...
public class ExcelNamedRangeCollection : IEnumerable<ExcelNamedRange>
{
    ...
    private void InsertColumns(int colFrom, int cols, ExcelNamedRange namedRange)
    {
        if (colFrom > 0)
        {
            ...
            {
                var newAddress = ExcelCellBase.GetAddress(namedRange.Start.Row, namedRange.Start.Column + cols,
                    namedRange.End.Row, namedRange.End.Column + cols);
            ...
            {
                var newAddress = ExcelCellBase.GetAddress(namedRange.Start.Row, namedRange.Start.Column,
                    namedRange.End.Row, namedRange.End.Column + cols);
                namedRange.Address = BuildNewAddress(namedRange, newAddress);
            }
        }
    ...
    private void InsertRows(int rowFrom, int rows, ExcelNamedRange namedRange)
    {
        if (rows > 0)
        {
            ...
            {
                var newAddress = ExcelCellBase.GetAddress(namedRange.Start.Row + rows, namedRange.Start.Column,
                    namedRange.End.Row + rows, namedRange.End.Column);
                namedRange.Address = BuildNewAddress(namedRange, newAddress);
            }
    ...
    internal void Clear()
    {
        while (Count > 0) Remove(_list[0].Name);
    }
