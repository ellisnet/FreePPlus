...
public sealed class ExcelStyles : XmlHelper
{
    ...
    private void SetStyleAddress(StyleBase sender, StyleChangeEventArgs e, ExcelAddressBase address, ExcelWorksheet ws,
        ref Dictionary<int, int> styleCashe)
    {
        if (address.Start.Column == 0 || address.Start.Row == 0) throw new Exception("error address");
        //Columns
        if (address.Start.Row == 1 && address.End.Row == ExcelPackage.MaxRows)
        {
            ...
            while (column.ColumnMin <= address.End.Column)
            {
                ...
                {
                    if (column._columnMax == address.End.Column) break;
            ...
            var cse = new CellsStoreEnumerator<ExcelCoreValue>(ws._values, 1, address._fromCol, address._toRow,
                address._toCol);
            while (cse.Next())
        ...
        else if (address.Start.Column == 1 && address.End.Column == ExcelPackage.MaxColumns)
        {
            ...
            var cse2 = new CellsStoreEnumerator<ExcelCoreValue>(ws._values, address._fromRow, address._fromCol,
        ...
        else //Cellrange
        {
            ...
            ws._values.SetRangeValueSpecial(address.Start.Row, address.Start.Column, address.End.Row,
                address.End.Column,
    ...
    private void AddNewStyleColumn(StyleBase sender, StyleChangeEventArgs e, ExcelWorksheet ws,
        Dictionary<int, int> styleCashe, ExcelColumn column, int s)
    {
        if (styleCashe.ContainsKey(s))
        {
            ws.SetStyleInner(0, column.ColumnMin, styleCashe[s]);
    ...
    {
        var v = 0;
        if (ws.ExistsStyleInner(row, col, ref v)) return v;

        if (ws.ExistsStyleInner(row, 0, ref v)) //First Row
            return v;

        // then column
        if (ws.ExistsStyleInner(0, col, ref v)) return v;
    ...
    internal int NamedStylePropertyChange(StyleBase sender, StyleChangeEventArgs e)
    {
        ...
        if (index >= 0)
        {
            var newId = CellStyleXfs[NamedStyles[index].StyleXfId]
...
//                style.Style = new ExcelStyle(this, NamedStylePropertyChange, -1, name, 0);
    ...
    public void UpdateXml()
    {
        ...
        foreach (var border in Borders)
            if (border.useCnt > 0)
            {
                bordersNode.AppendChild(
                    border.CreateXmlNode(_styleXml.CreateElement("border", ExcelPackage.SchemaMain)));
                border.newID = count;
                count++;
            }
        ...
        if (NamedStyles.Count > 0) styleXfsNode.RemoveAll();
        //NamedStyles
        ...
        if (cellStyleNode != null) cellStyleNode.RemoveAll();
        ...
        foreach (var style in NamedStyles)
        {
            ...
            cellStyleNode.AppendChild(
                style.CreateXmlNode(_styleXml.CreateElement("cellStyle", ExcelPackage.SchemaMain)));
        }
    ...
    private void RemoveUnusedStyles()
    {
        ...
        foreach (var sheet in _wb.Worksheets)
        {
            ...
            while (cse.Next())
            {
                ...
                if (v >= 0) CellXfs[v].useCnt++;
            }
        }

        foreach (var ns in NamedStyles) CellStyleXfs[ns.StyleXfId].useCnt++;
    ...
    internal int GetStyleIdFromName(string Name)
    {
        ...
        if (i >= 0)
        {
            ...
            if (id < 0)
            {
                ...
                if (id < 0) id = CellXfs.Add(newStyle.Id, newStyle);
        ...
        lock (style)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (index < 0) index = CellXfs.Add(newXfs.Id, newXfs);
                }
            }
