...
public sealed class ExcelWorkbook : XmlHelper, IDisposable
{
        ...
        base(namespaceManager)
    {
        ...
        SchemaNodeOrder = new[]
        {
            ...
            "functionPrototypes", "externalReferences", "definedNames", "calcPr", "oleSize", "customWorkbookViews",
            "pivotCaches", "smartTagPr", "smartTagTypes", "webPublishing", "fileRecoveryPr", "webPublishObjects",
            "extLst"
        };
    ...
    ///     Note: Worksheets index either starts by 0 or 1 depending on the Excelpackage.Compatibility.IsWorksheets1Based
    ///     property.
    ...
    internal void GetDefinedNames()
    {
        ...
        if (nl != null)
            foreach (XmlElement elem in nl)
            {
                ...
                if (!int.TryParse(elem.GetAttribute("localSheetId"), NumberStyles.Number, CultureInfo.InvariantCulture,
                        out localSheetID))
                {
                    localSheetID = -1;
                    nameWorksheet = null;
                ...
                {
                    nameWorksheet = Worksheets[localSheetID + _package.WorksheetAddId];
                }
                    ...
                    if (start >= 0 && end >= 0)
                    {
                        ...
                        if (int.TryParse(externalIndex, NumberStyles.Any, CultureInfo.InvariantCulture, out index))
                            if (index > 0 && index <= _externalReferences.Count)
                                fullAddress = fullAddress[..start] + "[" + _externalReferences[index - 1] + "]" +
                                              fullAddress[(end + 1)..];
                    }
                }
                ...
                else
                {
                    ...
                    if (localSheetID > -1)
                    {
                        if (string.IsNullOrEmpty(addr._ws))
                            namedRange = Worksheets[localSheetID + _package.WorksheetAddId].Names
                                .Add(elem.GetAttribute("name"),
                                    new ExcelRangeBase(this, Worksheets[localSheetID + _package.WorksheetAddId],
                                        fullAddress, false));
                        else
                            namedRange = Worksheets[localSheetID + _package.WorksheetAddId].Names
                                .Add(elem.GetAttribute("name"),
                    ...
                    else
                    {
                        ...
                        namedRange = _names.Add(elem.GetAttribute("name"),
                            new ExcelRangeBase(this, ws, fullAddress, false));
                    }
                }
    ...
    internal void AddPivotTable(string cacheID, Uri defUri)
    {
        ...
        var rel = Part.CreateRelationship(UriHelper.ResolvePartUri(WorkbookUri, defUri), TargetMode.Internal,
    ...
    ///     <remarks>
    ///         This method uses GDI. If you use Asure or another environment that does not support GDI, you have to set
    ///         this value manually if you don't use the standard Calibri font
    ///     </remarks>
    /// </summary>
    ...
    internal static decimal GetWidthPixels(string fontName, float fontSize)
    {
        ...
        if (font.ContainsKey(fontSize)) return Convert.ToDecimal(font[fontSize].Width);

        float min = -1, max = 500;
        foreach (var size in font)
        {
            if (min < size.Key && size.Key < fontSize) min = size.Key;
            if (max > size.Key && size.Key > fontSize) max = size.Key;
        }

        if (min == max)
            return Convert.ToDecimal(font[min].Width);
        return Convert.ToDecimal(font[min].Height +
                                 (font[max].Height - font[min].Height) * ((fontSize - min) / (max - min)));
    }
    ...
    public ExcelWorkbookView View
    {
        get
        {
            if (_view == null) _view = new ExcelWorkbookView(NameSpaceManager, TopNode, this);
            return _view;
        }
    }
    ...
    public XmlDocument WorkbookXml
    {
        get
        {
            if (_workbookXml == null) CreateWorkbookXml(_namespaceManager);
            return _workbookXml;
        }
    }
    ...
    internal bool ExistsPivotCache(int cacheID, ref int newID)
    {
        ...
        foreach (var pt in ws.PivotTables)
        {
            if (pt.CacheID == cacheID) ret = false;
            if (pt.CacheID >= newID) newID = pt.CacheID + 1;
        }
    ...
    /// <summary>
    ///     The date systems used by Microsoft Excel can be based on one of two different dates. By default, a serial number of
    ///     1 in Microsoft Excel represents January 1, 1900.
    ///     The default for the serial number 1 can be changed to represent January 2, 1904.
    ///     This option was included in Microsoft Excel for Windows to make it compatible with Excel for the Macintosh, which
    ///     defaults to January 2, 1904.
    /// </summary>
    public bool Date1904
    {
        get
        {
            ...
            if (date1904Cache == null) date1904Cache = GetXmlNodeBool(date1904Path, false);
    ...
    private void CreateWorkbookXml(XmlNamespaceManager namespaceManager)
    {
        ...
        else
        {
            ...
            var partWorkbook = _package.Package.CreatePart(WorkbookUri,
                @"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet.main+xml", _package.Compression);
        ...
        get
        {
            if (_stylesXml == null)
            {
                ...
                else
                {
                    ...
                    var part = _package.Package.CreatePart(StylesUri,
                    ...
                    _package.Workbook.Part.CreateRelationship(UriHelper.GetRelativeUri(WorkbookUri, StylesUri),
                        TargetMode.Internal, ExcelPackage.SchemaRelationships + "/styles");
                    _package.Package.Flush();
                }
            }
    ...
    public ExcelStyles Styles
    {
        get
        {
            if (_styles == null) _styles = new ExcelStyles(NameSpaceManager, StylesXml, this);
            return _styles;
        }
    }
    ...
    ///     <remarks>
    ///         This property is always true for both new workbooks and loaded templates(on load). If this is not the
    ///         wanted behavior set this property to false.
    ///     </remarks>
    /// </summary>
    ...
    internal void Save() // Workbook Save
    {
        ...
        // save the workbook
        if (_workbookXml != null) _package.SavePart(WorkbookUri, _workbookXml);

        // save the properties of the workbook
        if (_properties != null) _properties.Save();
        ...
        {
            if (isProtected && Protection.LockWindows) worksheet.View.WindowProtection = true;
            worksheet.Save();
        ...
        {
            part = _package.Package.CreatePart(SharedStringsUri,
                @"application/vnd.openxmlformats-officedocument.spreadsheetml.sharedStrings+xml", _package.Compression);
            Part.CreateRelationship(UriHelper.GetRelativeUri(WorkbookUri, SharedStringsUri), TargetMode.Internal,
        ...
        if (name.IsName)
        {
            if (string.IsNullOrEmpty(name.NameFormula))
            {
                if (TypeCompat.IsPrimitive(name.NameValue) || name.NameValue is double || name.NameValue is decimal)
                    elem.InnerText = Convert.ToDouble(name.NameValue, CultureInfo.InvariantCulture)
    ...
    private bool ExistsNames()
    {
        if (_names.Count == 0)
            foreach (var ws in Worksheets)
            {
                ...
                if (ws.Names.Count > 0) return true;
            }
        else
            return true;
