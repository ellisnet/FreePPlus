...
internal class RangeCollection : IEnumerator<IRangeID>, IEnumerable, IDisposable
{
    ...
    internal RangeCollection(List<IRangeID> cells)
    {
        _cells = cells;
        InitSize(_cells);
        for (var i = 0; i < _cells.Count; i++) _cellIndex[i] = new IndexItem(cells[i].RangeID, i);
    }
    ...
    internal void Add(IRangeID cell)
    {
        ...
        if (ix >= 0) throw new Exception("Item already exists");
        Insert(~ix, cell);
    }
    ...
    {
        var ix = IndexOf(key);
        if (ix < 0) throw new Exception("Key does not exists");
    ...
    internal int InsertRowsUpdateIndex(ulong rowID, int rows)
    {
        ...
        for (var i = index; i < _cells.Count; i++) _cellIndex[i].RangeID += rowAdd;
        return index;
    }
    ...
    internal int DeleteRows(ulong rowID, int rows, bool updateCells)
    {
        ...
        while (index < _cells.Count && _cellIndex[index].RangeID < rowID + rowAdd) Delete(_cellIndex[index].RangeID);
