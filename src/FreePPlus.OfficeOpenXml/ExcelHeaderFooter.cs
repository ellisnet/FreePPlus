...
public sealed class ExcelHeaderFooter : XmlHelper
{
    ...
    ///     Theme Color is specified as TTSNN where TT is the theme color Id, S is either "+" or "-" of the tint/shade value,
    ///     NN is the tint/shade value.
    /// </summary>
    ...
    public ExcelHeaderFooterText OddHeader
    {
        get
        {
            if (_oddHeader == null)
                _oddHeader =
                    new ExcelHeaderFooterText(TopNode.SelectSingleNode("d:oddHeader", NameSpaceManager), _ws, "H");
            return _oddHeader;
        }
    }
    ...
    public ExcelVmlDrawingPictureCollection Pictures
    {
        get
        {
            ...
            {
                var vmlNode =
                    _ws.WorksheetXml.SelectSingleNode("d:worksheet/d:legacyDrawingHF/@r:id", NameSpaceManager);
                if (vmlNode == null)
                {
    ...
    internal void Save()
    {
        if (_oddHeader != null) SetXmlNodeString("d:oddHeader", GetText(OddHeader));
    ...
    internal void SaveHeaderFooterImages()
    {
        if (_vmlDrawingsHF != null)
        {
            ...
            else
            {
                ...
                {
                    _vmlDrawingsHF.Part = _ws._package.Package.CreatePart(_vmlDrawingsHF.Uri,
                        "application/vnd.openxmlformats-officedocument.vmlDrawing", _ws._package.Compression);
                    var rel = _ws.Part.CreateRelationship(
                        UriHelper.GetRelativeUri(_ws.WorksheetUri, _vmlDrawingsHF.Uri), TargetMode.Internal,
                    ...
                    {
                        rel = _vmlDrawingsHF.Part.CreateRelationship(
                            UriHelper.GetRelativeUri(_vmlDrawingsHF.Uri, draw.ImageUri), TargetMode.Internal,
