...
/// </summary>
public sealed class ExcelBubbleChartSeriesItem : ExcelChartSeriesItem
{
    ...
    internal ExcelBubbleChartSeriesItem(ExcelChartSeries chartSeries, XmlNamespaceManager ns, XmlNode node,
        bool isPivot) :
    ...
    public override string Series
    {
        ...
        {
            base.Series = value;
            if (string.IsNullOrEmpty(BubbleSize)) GenerateLit();
        }
    }
    ...
    public string BubbleSize
    {
        ...
        set
        {
            ...
            {
                SetXmlNodeString(BUBBLESIZE_PATH,
                    ExcelCellBase.GetFullAddress(_chartSeries.Chart.WorkSheet.Name, value));

                var cache = TopNode.SelectSingleNode(string.Format("{0}/c:numCache", BUBBLESIZE_PATH), _ns);
                if (cache != null) cache.ParentNode.RemoveChild(cache);
