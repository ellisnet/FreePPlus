...
{
    private readonly List<ExcelChartTrendline> _list = new();
    private readonly ExcelChartSeriesItem _serie;

    internal ExcelChartTrendlineCollection(ExcelChartSeriesItem serie)
    {
        _serie = serie;
    ...
    public ExcelChartTrendline Add(eTrendLine Type)
    {
        if (_serie._chartSeries._chart.IsType3D() ||
            ...
            throw new ArgumentException(
                "Trendlines don't apply to 3d-charts, stacked charts, pie charts or doughnut charts");
...
public class ExcelChartTrendline : XmlHelper
{
...

    {
        SchemaNodeOrder = new[]
        {
            "name", "trendlineType", "order", "period", "forward", "backward", "intercept", "dispRSqr", "dispEq",
            "trendlineLbl"
        };
    }
