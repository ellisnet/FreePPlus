...
public class ExcelDrawings : IEnumerable<ExcelDrawing>, IDisposable
{
    ...
    public void Dispose()
    {
        ...
        foreach (var d in _drawings) d.Dispose();
        _drawings.Clear();
        _drawings = null;
    }
    ...
    private void AddDrawings()
    {
        ...
        var list = DrawingXml.SelectNodes(
            "//*[self::xdr:twoCellAnchor or self::xdr:oneCellAnchor or self::xdr:absoluteAnchor]", NameSpaceManager);
        ...
        foreach (XmlNode node in list)
        {
            ...
            {
                _drawings.Add(dr);
                if (!_drawingNames.ContainsKey(dr.Name)) _drawingNames.Add(dr.Name, _drawings.Count - 1);
            }
        }
    }
    ...
    internal void AdjustWidth(int[,] pos)
    {
        ...
        foreach (ExcelDrawing d in this)
        {
            ...
            {
                if (d.EditAs == eEditAs.Absolute) d.SetPixelLeft(pos[ix, 0]);
    ...
    internal void AdjustHeight(int[,] pos)
    {
        ...
        foreach (ExcelDrawing d in this)
        {
            if (d.EditAs != eEditAs.TwoCell)
            {
                if (d.EditAs == eEditAs.Absolute) d.SetPixelTop(pos[ix, 0]);
    ...
    internal class ImageCompare
    {
        ...
        {
            if (compareImg.Length != image.Length) return false;
    ...
    {
        if (_drawingNames.ContainsKey(Name)) throw new Exception("Name already exists in the drawings collection");
    ...
    private XmlElement CreateDrawingXml()
    {
        if (DrawingXml.DocumentElement == null)
        {
            ...
            Part = package.CreatePart(UriDrawing, "application/vnd.openxmlformats-officedocument.drawing+xml",
            ...
            _drawingRelation = Worksheet.Part.CreateRelationship(
                UriHelper.GetRelativeUri(Worksheet.WorksheetUri, UriDrawing), TargetMode.Internal,
        ...
        var colNode = DrawingXml.SelectSingleNode("//xdr:wsDr", NameSpaceManager);
        XmlElement drawNode;
        if (Worksheet is ExcelChartsheet)
        {
            drawNode = DrawingXml.CreateElement("xdr", "absoluteAnchor", ExcelPackage.SchemaSheetDrawings);
            var posNode = DrawingXml.CreateElement("xdr", "pos", ExcelPackage.SchemaSheetDrawings);
            posNode.SetAttribute("y", "0");
            posNode.SetAttribute("x", "0");
            drawNode.AppendChild(posNode);
            var extNode = DrawingXml.CreateElement("xdr", "ext", ExcelPackage.SchemaSheetDrawings);
        ...
        else
        {
            ...
            var fromNode = DrawingXml.CreateElement("xdr", "from", ExcelPackage.SchemaSheetDrawings);
            ...
            var toNode = DrawingXml.CreateElement("xdr", "to", ExcelPackage.SchemaSheetDrawings);
    ...
    internal void RemoveDrawing(int Index)
    {
        ...
        draw.DeleteMe();
        for (var i = Index + 1; i < _drawings.Count; i++) _drawingNames[_drawings[i].Name]--;
