...
public class ExcelDrawing : XmlHelper, IDisposable
{
        ...
        base(drawings.NameSpaceManager, node)
    {
        ...
        if (node != null) From = new ExcelPosition(drawings.NameSpaceManager, posNode, GetPositionSize);
    ...
    public eEditAs EditAs
    {
        ...
        set
        {
            ...
            SetXmlNodeString("@editAs", s[..1].ToLower(CultureInfo.InvariantCulture) + s.Substring(1, s.Length - 1));
        }
    }
    ...
    internal int GetPixelTop()
    {
        var ws = _drawings.Worksheet;
        var pix = 0;
        for (var row = 0; row < From.Row; row++) pix += (int)(GetRowHeight(row + 1) / 0.75);
    ...
    internal int GetPixelHeight()
    {
        ...
        for (var row = From.Row + 1; row <= To.Row; row++) pix += (int)(GetRowHeight(row) / 0.75);
    ...
    private double GetRowHeightFromCellFonts(int row, ExcelWorksheet ws)
    {
        ...
        if (double.IsNaN(dh) || ws.CustomHeight == false)
        {
            ...
            var cse = new CellsStoreEnumerator<ExcelCoreValue>(_drawings.Worksheet._values, row, 0, row,
    ...
    internal void SetPixelLeft(int pixels)
    {
        ...
        var mdw = ws.Workbook.MaxFontWidth;
        var prevPix = 0;
        var pix = (int)decimal.Truncate((256 * GetColumnWidth(1) + decimal.Truncate(128 / mdw)) / 256 * mdw);
        var col = 2;
    ...
    internal void SetPixelHeight(int pixels, double dpi)
    {
        _doNotAdjust = true;
        var ws = _drawings.Worksheet;
        //decimal mdw = ws.Workbook.MaxFontWidth;
        pixels = (int)(pixels / (dpi / STANDARD_DPI) + .5);
        var pixOff = pixels - ((int)(GetRowHeight(From.Row + 1) / 0.75) - From.RowOff / EMU_PER_PIXEL);
    ...
    internal void SetPixelWidth(int pixels, double dpi)
    {
        _doNotAdjust = true;
        var ws = _drawings.Worksheet;
        var mdw = ws.Workbook.MaxFontWidth;
        ...
        pixels = (int)(pixels / (dpi / STANDARD_DPI) + .5);
        var pixOff = pixels -
                     ((int)decimal.Truncate((256 * GetColumnWidth(From.Column + 1) + decimal.Truncate(128 / mdw)) /
                         256 * mdw) - From.ColumnOff / EMU_PER_PIXEL);
        var prevPixOff = From.ColumnOff / EMU_PER_PIXEL + pixels;
