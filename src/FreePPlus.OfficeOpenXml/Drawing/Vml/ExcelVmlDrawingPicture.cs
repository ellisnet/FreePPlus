...
public class ExcelVmlDrawingPicture : ExcelVmlDrawingBase
{
    ...
    private double GetFracDT(string v, double def)
    {
        ...
        {
            if (!double.TryParse(v, NumberStyles.Any, CultureInfo.InvariantCulture, out d)) d = def;
        }
    ...
    private void SetStyleProp(string propertyName, string value)
    {
        ...
        if (!found) newStyle += propertyName + ":" + value + ";";
        SetXmlNodeString("@style", newStyle[..^1]);
    }
    ...
    private double GetStyleProp(string propertyName)
    {
        ...
        foreach (var prop in style.Split(';'))
        {
            ...
            {
                var value = split[1].EndsWith("pt") ? split[1][..^2] : split[1];
                double ret;
