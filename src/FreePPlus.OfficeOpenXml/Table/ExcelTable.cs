...
public class ExcelTable : XmlHelper, IEqualityComparer<ExcelTable>
{
        ...
        base(sheet.NameSpaceManager)
    {
        ...
        if (address._fromRow == address._toRow) ShowHeader = false;
    }
    ...
    public string Name
    {
        ...
        set
        {
            if (Name.Equals(value, StringComparison.CurrentCultureIgnoreCase) == false &&
                WorkSheet.Workbook.ExistsTableName(value)) throw new ArgumentException("Tablename is not unique");
            var prevName = Name;
    ...
    public ExcelTableColumnCollection Columns
    {
        get
        {
            if (_cols == null) _cols = new ExcelTableColumnCollection(this);
            return _cols;
        }
    }
    ...
    public TableStyles TableStyle
    {
        ...
        {
            _tableStyle = value;
            if (value != TableStyles.Custom) SetXmlNodeString(STYLENAME_PATH, "TableStyle" + value);
        }
    }
        ...
        set
        {
            ...
            if (value)
            {
                ...
                for (var i = 0; i < Columns.Count; i++)
                {
                    ...
                    else if (v != _cols[i].Name) _cols[i].Name = v;
                }
            }
            else
            {
    ...
    public bool ShowTotal
    {
        ...
        set
        {
            ...
            {
                if (value)
                    Address = new ExcelAddress(WorkSheet.Name,
                        ExcelCellBase.GetAddress(Address.Start.Row, Address.Start.Column, Address.End.Row + 1,
                            Address.End.Column));
                else
                    Address = new ExcelAddress(WorkSheet.Name,
                        ExcelCellBase.GetAddress(Address.Start.Row, Address.Start.Column, Address.End.Row - 1,
    ...
    public string DataCellStyleName
    {
        ...
        set
        {
            ...
            if (fromRow < toRow) WorkSheet.Cells[fromRow, Address._fromCol, toRow, Address._toCol].StyleName = value;
        }
    }
    ...
    private void WriteAutoFilter(bool showTotal)
    {
        ...
        {
            if (showTotal)
                autofilterAddress = ExcelCellBase.GetAddress(Address._fromRow, Address._fromCol, Address._toRow - 1,
                    Address._toCol);
            else
