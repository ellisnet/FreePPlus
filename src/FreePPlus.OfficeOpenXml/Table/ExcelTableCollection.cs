...
public class ExcelTableCollection : IEnumerable<ExcelTable>
{
    ...
    public ExcelTable this[int Index]
    {
        get
        {
            if (Index < 0 || Index >= _tables.Count) throw new ArgumentOutOfRangeException("Table index out of range");
    ...
    private ExcelTable Add(ExcelTable tbl)
    {
        ...
        if (tbl.Id >= _ws.Workbook._nextTableID) _ws.Workbook._nextTableID = tbl.Id + 1;
        return tbl;
    }
    ...
    public ExcelTable Add(ExcelAddressBase Range, string Name)
    {
        ...
        {
            if (_ws.Workbook.ExistsTableName(Name)) throw new ArgumentException("Tablename is not unique");
        }
    ...
    {
        if (string.IsNullOrEmpty(Name)) throw new ArgumentException("Tablename is null or empty");
        ...
        if (Name.Contains(" ")) throw new ArgumentException("Tablename has spaces");
        if (!ExcelAddressUtil.IsValidName(Name)) throw new ArgumentException("Tablename is not valid");
    }
    ...
    {
        if (this[Name] == null)
            throw new ArgumentOutOfRangeException(string.Format("Cannot delete non-existant table {0} in sheet {1}.",
                Name, _ws.Name));
    ...
    public void Delete(ExcelTable Table, bool ClearRange = false)
    {
        if (!_tables.Contains(Table))
            throw new ArgumentOutOfRangeException("Table",
                string.Format("Table {0} does not exist in this collection", Table.Name));
        lock (this)
        {
    ...
    internal string GetNewTableName()
    {
        var name = "Table1";
        var i = 2;
        while (_ws.Workbook.ExistsTableName(name)) name = string.Format("Table{0}", i++);
        return name;
    }
