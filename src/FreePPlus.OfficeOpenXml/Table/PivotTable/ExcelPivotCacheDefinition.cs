...
/// <summary>
///     Cache definition. This class defines the source data. Note that one cache definition can be shared between many
///     pivot tables.
/// </summary>
public class ExcelPivotCacheDefinition : XmlHelper
{
    ...
    internal ExcelPivotCacheDefinition(XmlNamespaceManager ns, ExcelPivotTable pivotTable) :
        base(ns, null)
    {
        foreach (var r in pivotTable.Part.GetRelationshipsByType(ExcelPackage.SchemaRelationships +
                                                                 "/pivotCacheDefinition")) Relationship = r;
        ...
        if (CacheSource == eSourceType.Worksheet)
        {
            ...
            if (pivotTable.WorkSheet.Workbook.Worksheets.Any(t => t.Name == worksheetName))
                _sourceRange = pivotTable.WorkSheet.Workbook.Worksheets[worksheetName]
                    .Cells[GetXmlNodeString(_sourceAddressPath)];
        }
    }

    internal ExcelPivotCacheDefinition(XmlNamespaceManager ns, ExcelPivotTable pivotTable, ExcelRangeBase sourceAddress,
        int tblId) :
        base(ns, null)
    {
        PivotTable = pivotTable;
        ...
        RecordRelationship = Part.CreateRelationship(UriHelper.ResolvePartUri(CacheDefinitionUri, CacheRecordUri),
            TargetMode.Internal, ExcelPackage.SchemaRelationships + "/pivotCacheRecords");
    ...
    private string GetStartXml(ExcelRangeBase sourceAddress)
    {
        ...
        xml += string.Format("<worksheetSource ref=\"{0}\" sheet=\"{1}\" /> ", sourceAddress.Address,
        ...
        {
            if (sourceWorksheet == null || sourceWorksheet.GetValueInner(sourceAddress._fromRow, col) == null ||
                sourceWorksheet.GetValueInner(sourceAddress._fromRow, col).ToString().Trim() == "")
                xml += string.Format("<cacheField name=\"Column{0}\" numFmtId=\"0\">",
                    col - sourceAddress._fromCol + 1);
            else
                xml += string.Format("<cacheField name=\"{0}\" numFmtId=\"0\">",
                    SecurityElement.Escape(sourceWorksheet.GetValueInner(sourceAddress._fromRow, col).ToString()));
