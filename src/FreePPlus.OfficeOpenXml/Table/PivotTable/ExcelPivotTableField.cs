...
public class ExcelPivotTableField : XmlHelper
{
    ...
    internal ExcelPivotTableField(XmlNamespaceManager ns, XmlNode topNode, ExcelPivotTable table, int index,
        int baseIndex) :
        base(ns, topNode)
    {
        Index = index;
            ...
            foreach (XmlAttribute item in nl)
                ...
                catch (ArgumentException ex)
                {
                    throw new ArgumentException(
                        "Unable to parse value of " + item.Value + " to a valid pivot table subtotal function", ex);
                }
            ...
            else
            {
                ...
                foreach (eSubTotalFunctions e in Enum.GetValues(typeof(eSubTotalFunctions)))
                    if ((value & e) == e)
                    {
                        ...
                        var totalType = char.ToLowerInvariant(newTotalType[0]) + newTotalType[1..];
                        // add new attribute
    ...
    {
        get => TopNode.SelectSingleNode(string.Format("../../d:rowFields/d:field[@x={0}]", Index), NameSpaceManager) !=
               null;
        internal set
        {
            if (value)
            {
                ...
                if (rowsNode == null) _table.CreateNode("d:rowFields");
            ...
            {
                var node = TopNode.SelectSingleNode(string.Format("../../d:rowFields/d:field[@x={0}]", Index),
    ...
    {
        get => TopNode.SelectSingleNode(string.Format("../../d:colFields/d:field[@x={0}]", Index), NameSpaceManager) !=
               null;
        internal set
        {
            if (value)
            {
                ...
                if (columnsNode == null) _table.CreateNode("d:colFields");
            ...
            {
                var node = TopNode.SelectSingleNode(string.Format("../../d:colFields/d:field[@x={0}]", Index),
                    NameSpaceManager) as XmlElement;
                if (node != null) node.ParentNode.RemoveChild(node);
            }
        }
    ...
    public bool IsPageField
    {
        ...
        internal set
        {
            ...
            else
            {
                ...
                var node = TopNode.SelectSingleNode(string.Format("../../d:pageFields/d:pageField[@fld={0}]", Index),
                    NameSpaceManager) as XmlElement;
                if (node != null) node.ParentNode.RemoveChild(node);
            }
        }
        ...
        get
        {
            if (_items == null)
            {
                ...
                foreach (XmlNode node in TopNode.SelectNodes("d:items//d:item", NameSpaceManager))
                {
                    ...
                    if (item.T == "") _items.AddInternal(item);
                }
    ...
    {
        if (!(IsColumnField || IsRowField)) throw new Exception("Field must be a row or column field");
        foreach (var field in _table.Fields)
            if (field.Grouping != null)
                throw new Exception("Grouping already exists");
    }

    private ExcelPivotTableField AddField(eDateGroupBy groupBy, DateTime startDate, DateTime endDate,
        ref bool firstField)
    {
    ...
    private ExcelPivotTableField AddField(eDateGroupBy groupBy, DateTime startDate, DateTime endDate,
        ref bool firstField, int interval)
    {
        if (firstField == false)
        {
            //Pivot field
            ...
            var field = new ExcelPivotTableField(_table.NameSpaceManager, fieldNode, _table, _table.Fields.Count,
                Index);
    ...
    private void AddCacheField(ExcelPivotTableField field, DateTime startDate, DateTime endDate, int interval)
    {
        ...
        var cacheTopNode =
            _table.CacheDefinition.CacheDefinitionXml.SelectSingleNode("//d:cacheFields", _table.NameSpaceManager);
        var cacheFieldNode =
            _table.CacheDefinition.CacheDefinitionXml.CreateElement("cacheField", ExcelPackage.SchemaMain);
        ...
        int interval)
    {
        ...
        group.TopNode.InnerXml +=
            string.Format("<fieldGroup base=\"{0}\"><rangePr groupBy=\"{1}\" /><groupItems /></fieldGroup>", BaseIndex,
        ...
        {
            _cacheFieldHelper.SetXmlNodeString("d:fieldGroup/d:rangePr/@startDate",
        ...
        {
            _cacheFieldHelper.SetXmlNodeString("d:fieldGroup/d:rangePr/@endDate",
    ...
    internal ExcelPivotTableFieldNumericGroup SetNumericGroup(double start, double end, double interval)
    {
        ...
        group.TopNode.InnerXml += string.Format(
            ...
            BaseIndex, start.ToString(CultureInfo.InvariantCulture), end.ToString(CultureInfo.InvariantCulture),
    ...
    {
        if (interval < 0) throw new Exception("The interval must be a positiv");
        if (start > end) throw new Exception("Then End number must be larger than the Start number");

        var groupItems =
            group.TopNode.SelectSingleNode("d:fieldGroup/d:groupItems", group.NameSpaceManager) as XmlElement;
        var items = 2;
        //First date
        var index = start;
        ...
        while (index < end)
        {
            AddGroupItem(groupItems,
                string.Format("{0}-{1}", index.ToString(CultureInfo.InvariantCulture),
    ...
    private int AddDateGroupItems(ExcelPivotTableFieldGroup group, eDateGroupBy GroupBy, DateTime StartDate,
        DateTime EndDate, int interval)
    {
        var groupItems =
            group.TopNode.SelectSingleNode("d:fieldGroup/d:groupItems", group.NameSpaceManager) as XmlElement;
        var items = 2;
        //First date
        AddGroupItem(groupItems, "<" + StartDate.ToString("s", CultureInfo.InvariantCulture)[..10]);
        ...
        //Lastdate
        AddGroupItem(groupItems, ">" + EndDate.ToString("s", CultureInfo.InvariantCulture)[..10]);
        return items;
    }

    private void AddTimeSerie(int count, XmlElement groupItems)
    {
        for (var i = 0; i < count; i++) AddGroupItem(groupItems, string.Format("{0:00}", i));
    }
