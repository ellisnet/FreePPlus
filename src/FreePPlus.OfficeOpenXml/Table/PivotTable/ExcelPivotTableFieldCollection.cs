...
public class ExcelPivotTableFieldCollectionBase<T> : IEnumerable<T>
{
    ...
    public T this[int Index]
    {
        get
        {
            if (Index < 0 || Index >= _list.Count) throw new ArgumentOutOfRangeException("Index out of range");
            return _list[Index];
        }
    }
...
public class ExcelPivotTableRowColumnFieldCollection : ExcelPivotTableFieldCollectionBase<ExcelPivotTableField>
{
    ...
    {
        if (!_list.Contains(Field)) throw new ArgumentException("Field not in collection");
        SetFlag(Field, false);
        _list.Remove(Field);
    }
    ...
    {
        if (Index > -1 && Index < _list.Count) throw new IndexOutOfRangeException();
...
public class ExcelPivotTableDataFieldCollection : ExcelPivotTableFieldCollectionBase<ExcelPivotTableDataField>
{
    ...
    {
        var node = dataField.Field.TopNode.SelectSingleNode(
