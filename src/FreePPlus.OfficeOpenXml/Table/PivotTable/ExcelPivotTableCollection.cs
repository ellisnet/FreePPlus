...
public class ExcelPivotTableCollection : IEnumerable<ExcelPivotTable>
{
    ...
    private ExcelPivotTable Add(ExcelPivotTable tbl)
    {
        ...
        if (tbl.CacheID >= _ws.Workbook._nextPivotTableID) _ws.Workbook._nextPivotTableID = tbl.CacheID + 1;
        return tbl;
    }
    ...
    {
        if (string.IsNullOrEmpty(Name)) Name = GetNewTableName();
    ...
    internal string GetNewTableName()
    {
        ...
        var i = 2;
        while (_ws.Workbook.ExistsPivotTableName(name)) name = string.Format("Pivottable{0}", i++);
        return name;
    }
}