...
public class ExcelTableColumn : XmlHelper
{
    ...
    public string Name
    {
        get
        {
            ...
            {
                if (_tbl.ShowHeader)
                    n = ConvertUtil.ExcelDecodeString(_tbl.WorkSheet.GetValue<string>(_tbl.Address._fromRow,
    ...
    public string DataCellStyleName
    {
        ...
        set
        {
            ...
            if (fromRow <= toRow) _tbl.WorkSheet.Cells[fromRow, col, toRow, col].StyleName = value;
        }
    }
    ...
    ///     Be carefull with this property since it is not validated.
    ///     <example>
    ///         tbl.Columns[9].CalculatedColumnFormula = string.Format("SUM(MyDataTable[[#This
    ///         Row],[{0}]])",tbl.Columns[9].Name);
    ///     </example>
    /// </summary>
