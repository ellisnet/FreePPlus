...
{
    private readonly Dictionary<string, int> _colNames = new(StringComparer.OrdinalIgnoreCase);
    private readonly List<ExcelTableColumn> _cols = new();

    public ExcelTableColumnCollection(ExcelTable table)
    {
        Table = table;
        foreach (XmlNode node in table.TableXml.SelectNodes("//d:table/d:tableColumns/d:tableColumn",
    ...
    public ExcelTableColumn this[int Index]
    {
        get
        {
            if (Index < 0 || Index >= _cols.Count) throw new ArgumentOutOfRangeException("Column index out of range");
            return _cols[Index];
        }
    }
