...
public class ExcelCommentCollection : IEnumerable, IDisposable
{
    ...
    private readonly List<ExcelComment> _list = new();
    ...
    public ExcelComment this[int Index]
    {
        get
        {
            if (Index < 0 || Index >= _list.Count) throw new ArgumentOutOfRangeException("Comment index out of range");
            return _list[Index];
        }
    }
    ...
    private void AddCommentsFromXml()
    {
        ...
        {
            var comment = new ExcelComment(NameSpaceManager, node,
                new ExcelRangeBase(Worksheet, node.GetAttribute("ref")));
            //lst.Add(comment);
            _list.Add(comment);
    ...
    public void Remove(ExcelComment comment)
    {
        ...
        var i = -1;
        ExcelComment c = null;
        if (Worksheet._commentsStore.Exists(comment.Range._fromRow, comment.Range._fromCol, ref i)) c = _list[i];
        if (comment == c)
        {
            ...
            Worksheet._commentsStore.Delete(comment.Range._fromRow, comment.Range._fromCol, 1, 1,
                false); //Issue 15549, Comments should not be shifted 
    ...
    internal void Delete(int fromRow, int fromCol, int rows, int columns)
    {
        ...
        foreach (var comment in _list)
        {
            ...
            if (fromCol > 0 && address._fromCol >= fromCol) address = address.DeleteColumn(fromCol, columns);
            if (fromRow > 0 && address._fromRow >= fromRow) address = address.DeleteRow(fromRow, rows);
        ...
        foreach (var comment in deletedComments) Remove(comment);
    }
