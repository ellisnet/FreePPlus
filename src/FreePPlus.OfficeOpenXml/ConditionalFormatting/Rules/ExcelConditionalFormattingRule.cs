        ...
        IExcelConditionalFormattingRule
{
            ...
            itemElementNode)
    {
        ...
        if (DxfId >= 0)
        {
            worksheet.Workbook.Styles.Dxfs[DxfId].AllowChange =
                true; //This Id is referenced by CF, so we can use it when we save.
            _style = worksheet.Workbook.Styles.Dxfs[DxfId]
                .Clone(); //Clone, so it can be altered without effecting other dxf styles
        }
    }
    ...
    {
        get => new(Node.ParentNode?.Attributes?[ExcelConditionalFormattingConstants.Attributes.Sqref]?.Value);
        ...
        set
        {
            ...
            if (Address.Address != value.Address)
            {
                ...
                if (Node != null) TopNode = newParentNode.AppendChild(Node);
                ...
                if (!_changingPriority)
                {
                    ...
                    if (priority > value)
                        for (var i = priority - 1; i >= value; i--)
                        {
                            ...
                            if (cfRule != null) cfRule.Priority++;
                        }
                    else
                        for (var i = priority + 1; i <= value; i++)
                        {
