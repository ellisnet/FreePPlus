...
internal static class ExcelConditionalFormattingRuleType
{
    ...
    private static eExcelConditionalFormattingRuleType GetIconSetType(XmlNode topNode,
        XmlNamespaceManager nameSpaceManager)
    {
        var node = topNode.SelectSingleNode("d:iconSet/@iconSet", nameSpaceManager);
        if (node == null) return eExcelConditionalFormattingRuleType.ThreeIconSet;

        var v = node.Value;

        if (v is { Length: > 0 } && v[0] == '3')
            return eExcelConditionalFormattingRuleType.ThreeIconSet;
        if (v is { Length: > 0 } && v[0] == '4')
