...
{
    private readonly byte[] BlockKey_HashInput = { 0xfe, 0xa7, 0xd2, 0x76, 0x3b, 0x4b, 0x9e, 0x79 };
    private readonly byte[] BlockKey_HashValue = { 0xd7, 0xaa, 0x0f, 0x6d, 0x30, 0x61, 0x34, 0x4e };

    private readonly byte[]
        BlockKey_HmacKey = { 0x5f, 0xb2, 0xad, 0x01, 0x0c, 0xb9, 0xe1, 0xf6 }; //MSOFFCRYPTO 2.3.4.14 section 3

    private readonly byte[]
        BlockKey_HmacValue = { 0xa0, 0x67, 0x7f, 0x02, 0xb2, 0x2c, 0x84, 0x33 }; //MSOFFCRYPTO 2.3.4.14 section 5

    private readonly byte[] BlockKey_KeyValue = { 0x14, 0x6e, 0x0b, 0xe7, 0xab, 0xac, 0xd0, 0xd6 };
    ...
    private MemoryStream EncryptPackageAgile(byte[] package, ExcelEncryption encryption)
    {
        ...
        var baseHash = GetPasswordHash(hashProvider, encr.SaltValue, encryption.Password, encr.SpinCount,
            encr.HashSize);
        ...
        EncryptAgileFromKey(encr, VerifierInputKey, encr.VerifierHashInput, 0, encr.VerifierHashInput.Length,
            encr.SaltValue, ms);
    ...
    private MemoryStream DecryptAgile(EncryptionInfoAgile encryptionInfo, string password, long size,
        byte[] encryptedData, byte[] data)
    {
        ...
        if (encryptionInfo.KeyData.CipherAlgorithm == eCipherAlgorithm.AES)
        {
            ...
            //Decrypt
            encr.VerifierHashInput = DecryptAgileFromKey(encr, valInputKey, encr.EncryptedVerifierHashInput,
                encr.SaltSize, encr.SaltValue);
            encr.VerifierHash = DecryptAgileFromKey(encr, valHashKey, encr.EncryptedVerifierHash, encr.HashSize,
                encr.SaltValue);
            encr.KeyValue = DecryptAgileFromKey(encr, valKeySizeKey, encr.EncryptedKeyValue,
                encryptionInfo.KeyData.KeyBits / 8, encr.SaltValue);
            ...
            if (IsPasswordValid(hashProvider, encr))
            {
                ...
                var key = DecryptAgileFromKey(encryptionInfo.KeyData, encr.KeyValue,
                    encryptionInfo.DataIntegrity.EncryptedHmacKey, encryptionInfo.KeyData.HashSize, ivhmac);

                ivhmac = GetFinalHash(hashProviderDataKey, BlockKey_HmacValue, encryptionInfo.KeyData.SaltValue);
                var value = DecryptAgileFromKey(encryptionInfo.KeyData, encr.KeyValue,
                    encryptionInfo.DataIntegrity.EncryptedHmacValue, encryptionInfo.KeyData.HashSize, ivhmac);
    ...
    private MemoryStream DecryptBinary(EncryptionInfoBinary encryptionInfo, string password, long size,
        byte[] encryptedData)
    {
        var doc = new MemoryStream();

        if (encryptionInfo.Header.AlgID == AlgorithmID.AES128 || (encryptionInfo.Header.AlgID == AlgorithmID.Flags &&
                                                                  (encryptionInfo.Flags &
                                                                   (Flags.fAES | Flags.fExternal | Flags.fCryptoAPI)) ==
                                                                  (Flags.fAES | Flags.fCryptoAPI))
                                                              ||
    ...
    private byte[] DecryptAgileFromKey(EncryptionInfoAgile.EncryptionKeyData encr, byte[] key, byte[] encryptedData,
        long size, byte[] iv)
    {
    ...
    private SymmetricAlgorithm GetEncryptionAlgorithm(EncryptionInfoAgile.EncryptionKeyData encr)
    {
        switch (encr.CipherAlgorithm)
        {
            ...
            default:
                throw new NotSupportedException(string.Format("Unsupported Cipher Algorithm: {0}",
                    encr.CipherAlgorithm.ToString()));
        }
    }

    private void EncryptAgileFromKey(EncryptionInfoAgile.EncryptionKeyEncryptor encr, byte[] key, byte[] data, long pos,
        long size, byte[] iv, MemoryStream ms)
    {
    ...
    /// <returns>The hash to encrypt the document</returns>
    private byte[] GetPasswordHashAgile(string password, EncryptionInfoAgile.EncryptionKeyEncryptor encr,
        byte[] blockKey)
    {
        try
        {
    ...
    private byte[] GetPasswordHash(HashAlgorithm hashProvider, byte[] salt, string password, int spinCount,
        int hashSize)
    {
        byte[] hash = null;
    ...
    private byte[] FixHashSize(byte[] hash, int size, byte fill = 0)
    {
        ...
        if (hash.Length < size)
        {
            ...
            for (var i = hash.Length; i < size; i++) buff[i] = fill;
            return buff;
        }
        else
        {
