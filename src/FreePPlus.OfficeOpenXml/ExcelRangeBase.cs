...
///     A range of cells
/// </summary>
public class ExcelRangeBase : ExcelAddress, IExcelCell, IDisposable, IEnumerable<ExcelRangeBase>,
    IEnumerator<ExcelRangeBase>
{
        ...
        CompareOptions compareOptions = CompareOptions.None)
    {
        ...
        if (descending == null)
        {
            ...
            for (var i = 0; i < columns.Length; i++) descending[i] = false;
        }
        ...
        for (var c = 0; c < cols; c++)
        {
            ...
            //Move flags
            if (flags.ContainsKey(addr)) _worksheet._flags.SetValue(row, col, flags[addr]);
            //Move formulas
            if (formulas.ContainsKey(addr))
            {
                ...
                if (formulas[addr] is int)
                {
                    ...
                    {
                        f.Formula = TranslateFromR1C1(TranslateToR1C1(f.Formula, f.StartRow, f.StartCol), row,
                            f.StartCol);
                        f.StartRow = row;
            ...
            //Move hyperlinks
            if (hyperLinks.ContainsKey(addr)) _worksheet._hyperLinks.SetValue(row, col, hyperLinks[addr]);
    ...
    private static Dictionary<string, T> GetItems<T>(CellStore<T> store, int fromRow, int fromCol, int toRow, int toCol)
    {
        ...
        while (e.Next()) l.Add(e.CellAddress, e.Value);
        return l;
    }
        ...
        public int Compare(SortItem<ExcelCoreValue> x, SortItem<ExcelCoreValue> y)
        {
            ...
            for (var i = 0; i < columns.Length; i++)
            {
                ...
                if (isNumX && isNumY) //Numeric Compare
                {
                    ...
                    if (double.IsNaN(d1)) d1 = double.MaxValue;
                    if (double.IsNaN(d2)) d2 = double.MaxValue;
    ...
    {
        if (Table != null) SetRCFromTable(_workbook._package, null);
        SetDelegate();
    }
    ...
    private static void SetUnknown(ExcelRangeBase range, _setValue valueMethod, object value)
    {
        ...
        if (range._fromRow == -1) range.SetToSelectedRange();
        range.SetDelegate();
    ...
    {
        range.SetValueAddress(range, valueMethod, value);
        foreach (var address in range.Addresses) range.SetValueAddress(address, valueMethod, value);
    }
    ...
    private void SetValueAddress(ExcelAddress address, _setValue valueMethod, object value)
    {
        ...
        if (value is object[,] && valueMethod == Set_Value)
            ...
            _worksheet.SetRangeValueInner(address.Start.Row, address.Start.Column, address.End.Row, address.End.Column,
                (object[,])value);
        else
    ...
    private static void Set_Value(ExcelRangeBase range, object value, int row, int col)
    {
        ...
        if (sfi is int) range.SplitFormulas(range._worksheet.Cells[row, col]);
    ...
    private static void Set_SharedFormula(ExcelRangeBase range, string value, ExcelAddress address, bool IsArray)
    {
        ...
        if (address.Start.Row == address.End.Row && address.Start.Column == address.End.Column &&
    ...
    {
        var v = (string[])value;
        range._worksheet.Comments.Add(new ExcelRangeBase(range._worksheet, GetAddress(range._fromRow, range._fromCol)),
            v[0], v[1]);
    }
    ...
    public string StyleName
    {
        get
        {
            ...
            {
                xfId = 0;
                if (!_worksheet.ExistsStyleInner(_fromRow, 0, ref xfId)) xfId = GetColumnStyle(_fromCol);
            }
            else
            {
                xfId = 0;
        ...
        set
        {
            ...
            if (_fromRow == 1 && _toRow == ExcelPackage.MaxRows) //Full column
            {
                ...
                if (cols.Next())
                {
                    ...
                    while (column.ColumnMin <= _toCol)
                    {
                        ...
                        if (cols.Value._value == null) break;

                        var nextCol = (ExcelColumn)cols.Value._value;
                        if (column.ColumnMax < nextCol.ColumnMax - 1) column.ColumnMax = nextCol.ColumnMax - 1;
                        column = nextCol;
                        cols.Next();
                    }
                }

                if (column.ColumnMax < _toCol) column.ColumnMax = _toCol;

                if (_fromCol == 1 && _toCol == ExcelPackage.MaxColumns) //FullRow
                {
                    var rows = new CellsStoreEnumerator<ExcelCoreValue>(_worksheet._values, 1, 0, ExcelPackage.MaxRows,
                        0);
                    rows.Next();
            ...
            else //Only set name on created cells. (uncreated cells is set on full row or full column).
            {
                var cells = new CellsStoreEnumerator<ExcelCoreValue>(_worksheet._values, _fromRow, _fromCol, _toRow,
                    _toCol);
                while (cells.Next()) _worksheet.SetStyleInner(cells.Row, cells.Column, _styleID);
            }
        }
    }
    ...
    {
        object c = null;
        if (_worksheet.ExistsValueInner(0, col, ref c)) return (c as ExcelColumn).StyleID;

        var row = 0;
        if (_worksheet._values.PrevCell(ref row, ref col))
        {
            var column = _worksheet.GetValueInner(row, col) as ExcelColumn;
            if (column.ColumnMax >= col) return _worksheet.GetStyleInner(row, col);
        }
    ...
    private ExcelAddressBase GetAddressDim(ExcelRangeBase addr)
    {
        ...
        if (addr._fromRow == fromRow && addr._fromCol == fromCol && addr._toRow == toRow &&
            addr._toCol == _toCol) return addr;
    ...
    /// <summary>
    ///     Set the column width from the content of the range. The minimum width is the value of the
    ...
    public void AutoFitColumns(double minimumWidth, double maximumWidth)
    {
        ...
        if (_fromCol < 1 || _fromRow < 1) SetToSelectedRange();
        ...
        string textFormat)
    {
        ...
        {
            if (nf.DataType == ExcelNumberFormatXml.eFormatType.DateTime) return GetDateText((DateTime)v, format, nf);
    ...
    public string FormulaR1C1
    {
        ...
        set
        {
            ...
            {
                Set_SharedFormula(this, TranslateFromR1C1(value, _fromRow, _fromCol),
                    new ExcelAddress(WorkSheet, FirstAddress), false);
                foreach (var address in Addresses)
                    Set_SharedFormula(this, TranslateFromR1C1(value, address.Start.Row, address.Start.Column), address,
                        false);
            }
        }
    }
    ...
    public bool AutoFilter
    {
        ...
        set
        {
            ...
            if (_worksheet.Names.ContainsKey("_xlnm._FilterDatabase")) _worksheet.Names.Remove("_xlnm._FilterDatabase");
            if (value)
            {
    ...
    public ExcelRichTextCollection RichText
    {
        ...
        {
            IsRangeValid("richtext");
            if (_rtc == null) _rtc = GetRichText(_fromRow, _fromCol);
            return _rtc;
        }
    }
    ...
    private void SplitFormulas(ExcelAddressBase address)
    {
        ...
        foreach (var ix in formulas) SplitFormula(address, ix);
    ...
    private void SplitFormula(ExcelAddressBase address, int ix)
    {
        ...
        var firstCellCollide =
            address.Collide(new ExcelAddressBase(fRange._fromRow, fRange._fromCol, fRange._fromRow, fRange._fromCol));
    ...
    private object ConvertData(ExcelTextFormat Format, string v, int col, bool isText)
    {
        ...
        if (Format.DataTypes == null || Format.DataTypes.Length <= col || Format.DataTypes[col] == eDataTypes.Unknown)
        {
            ...
            if (DateTime.TryParse(v, Format.Culture, DateTimeStyles.None, out dt)) return dt;
    ...
    public IRangeConditionalFormatting ConditionalFormatting =>
        new RangeConditionalFormatting(_worksheet, new ExcelAddress(Address));
        ...
        TableStyles TableStyle = TableStyles.None)
    {
        ...
        if (rows >= 0 && r.Columns > 0)
        {
            var tbl = _worksheet.Tables.Add(
                new ExcelAddressBase(_fromRow, _fromCol, _fromRow + (rows <= 0 ? 1 : rows), _fromCol + r.Columns - 1),
                TableName);
    ...
    {
        if (Reader == null) throw new ArgumentNullException("Reader", "Reader can't be null");
        ...
        while (Reader.Read())
        {
            for (var i = 0; i < fieldCount; i++) _worksheet.SetValueInner(row, col++, Reader.GetValue(i));
            row++;
            col = _fromCol;
        }
    ...
    {
        if (Table == null) throw new ArgumentNullException("Table can't be null");

        if (Table.Rows.Count == 0 && PrintHeaders == false) return null;

        var rowArray = new List<object[]>();
        if (PrintHeaders) rowArray.Add(Table.Columns.Cast<DataColumn>().Select(dc => { return dc.Caption; }).ToArray());
        foreach (DataRow dr in Table.Rows) rowArray.Add(dr.ItemArray);
    ...
    {
        return LoadFromCollection(Collection, false, TableStyles.None, BindingFlags.Public | BindingFlags.Instance,
            null);
    }
    ...
    ///     Print the property names on the first row. If the property is decorated with a
    ///     <see cref="DisplayNameAttribute" /> or a <see cref="DescriptionAttribute" /> that attribute will be used instead of
    ///     the reflected member name.
    /// </param>
    /// <returns>The filled range</returns>
    public ExcelRangeBase LoadFromCollection<T>(IEnumerable<T> Collection, bool PrintHeaders)
    {
        return LoadFromCollection(Collection, PrintHeaders, TableStyles.None,
    ...
    {
        return LoadFromCollection(Collection, PrintHeaders, TableStyle, BindingFlags.Public | BindingFlags.Instance,
            null);
    }
    ...
    /// <param name="Collection">The collection to load</param>
    /// <param name="PrintHeaders">
    ///     Print the property names on the first row. Any underscore in the property name will be
    ///     converted to a space. If the property is decorated with a <see cref="DisplayNameAttribute" /> or a
    ///     <see cref="DescriptionAttribute" /> that attribute will be used instead of the reflected member name.
    /// </param>
    /// <param name="TableStyle">Will create a table with this style. If set to TableStyles.None no table will be created</param>
    ...
    /// <returns>The filled range</returns>
    public ExcelRangeBase LoadFromCollection<T>(IEnumerable<T> Collection, bool PrintHeaders, TableStyles TableStyle,
        BindingFlags memberFlags, MemberInfo[] Members)
    {
        var type = typeof(T);
        var isSameType = true;
        if (Members == null)
        ...
        else
        {
            ...
            {
                if (t.DeclaringType != null && t.DeclaringType != type) isSameType = false;
                //Fixing inverted check for IsSubclassOf / Pullrequest from tomdam
                if (t.DeclaringType != null && t.DeclaringType != type &&
                    !TypeCompat.IsSubclassOf(type, t.DeclaringType) && !TypeCompat.IsSubclassOf(t.DeclaringType, type))
        ...
        if (!Collection.Any() && (Members.Length == 0 || PrintHeaders == false)) return null;
            ...
            if (item is string || item is decimal || item is DateTime || TypeCompat.IsPrimitive(item))
                ...
                foreach (var t in Members)
                    if (isSameType == false && item.GetType().GetMember(t.Name, memberFlags).Length == 0)
                        col++;
                    //Check if the property exists if and inherited class is used
                    ...
                    else if (t is MethodInfo) values[row, col++] = ((MethodInfo)t).Invoke(item, null);
    ...
    public ExcelRangeBase LoadFromText(string Text, ExcelTextFormat Format)
    {
        ...
        foreach (var line in lines)
        {
            ...
            if (lineNo > Format.SkipLinesBeginning && lineNo <= lines.Length - Format.SkipLinesEnd)
            {
                ...
                if (QCount > 1 && v != "" && QCount == 2) v += new string(Format.TextQualifier, QCount / 2);
                if (lineQCount % 2 == 1)
    ...
    /// </returns>
    /// <remarks>
    ///     If  <see cref="Value" /> is string, parsing is performed for output types of DateTime and TimeSpan, which if fails
    ...
    ///     <see cref="DateTime.FromOADate" />
    ///     is used for conversion. This special case does not work through other types convertible to double (e.g. integer or
    ///     string with number).
    ...
    {
        if (_fromRow + RowOffset < 1 || _fromCol + ColumnOffset < 1 || _fromRow + RowOffset > ExcelPackage.MaxRows ||
            _fromCol + ColumnOffset > ExcelPackage.MaxColumns)
            throw new ArgumentOutOfRangeException("Offset value out of range");
        var address = GetAddress(_fromRow + RowOffset, _fromCol + ColumnOffset, _toRow + RowOffset,
            _toCol + ColumnOffset);
    ...
    public ExcelRangeBase Offset(int RowOffset, int ColumnOffset, int NumberOfRows, int NumberOfColumns)
    {
        ...
        if (_fromRow + RowOffset < 1 || _fromCol + ColumnOffset < 1 || _fromRow + RowOffset > ExcelPackage.MaxRows ||
            ...
            throw new ArgumentOutOfRangeException("Offset value out of range");
        var address = GetAddress(_fromRow + RowOffset, _fromCol + ColumnOffset, _fromRow + RowOffset + NumberOfRows,
    ...
    /// </summary>
    /// <param name="Destination">The start cell where the range will be copied.</param>
    /// <param name="excelRangeCopyOptionFlags">
    ///     Cell parts that will not be copied. If Formulas are specified, the formulas
    ///     will NOT be copied.
    /// </param>
    public void Copy(ExcelRangeBase Destination, ExcelRangeCopyOptionFlags? excelRangeCopyOptionFlags)
    {
        ...
        while (cse.Next())
        {
            ...
            if (_worksheet._hyperLinks.Exists(row, col, ref hl)) cell.HyperLink = hl;

            // Will just be null if no comment exists.
            cell.Comment = _worksheet.Cells[cse.Row, cse.Column].Comment;

            if (_worksheet._flags.Exists(row, col, ref flag)) cell.Flag = flag;
        ...
        foreach (var cell in copiedValue)
        {
            ...
            if (cell.StyleID != null) Destination._worksheet.SetStyleInner(cell.Row, cell.Column, cell.StyleID.Value);
            ...
            if (cell.Flag != 0) Destination._worksheet._flags.SetValue(cell.Row, cell.Column, cell.Flag);
        }
    ...
    {
        if (Addresses != null) throw new Exception("An Arrayformula can not have more than one address");
    ...
    private void DeleteCheckMergedCells(ExcelAddressBase Range)
    {
        ...
        foreach (var item in removeItems) Worksheet.MergedCells.Remove(item);
    }
