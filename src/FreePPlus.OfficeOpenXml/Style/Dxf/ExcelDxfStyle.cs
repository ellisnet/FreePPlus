...
public class ExcelDxfStyleConditionalFormatting : DxfStyleBase<ExcelDxfStyleConditionalFormatting>
{
    ...
    internal ExcelDxfStyleConditionalFormatting(XmlNamespaceManager nameSpaceManager, XmlNode topNode,
        ExcelStyles styles) : base(styles)
    {
    ...
    protected internal override string Id =>
        ...
        (AllowChange
            ? ""
            : DxfId.ToString()); //If allowchange is false we add the dxfID to ensure it's not used when conditional formatting is updated);
    ...
    private ExcelBorderStyle GetBorderStyleEnum(string style)
    {
        ...
        var sInStyle = style[..1].ToUpper(CultureInfo.InvariantCulture) + style.Substring(1, style.Length - 1);
        try
        {
    ...
    private ExcelFillStyle GetPatternTypeEnum(string patternType)
    {
        ...
        patternType = patternType[..1].ToUpper(CultureInfo.InvariantCulture) +
