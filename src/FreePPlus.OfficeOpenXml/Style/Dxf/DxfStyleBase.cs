...
public abstract class DxfStyleBase<T>
{
    ...
    internal DxfStyleBase(ExcelStyles styles)
    {
        _styles = styles;
        AllowChange =
            false; //Don't touch this value in the styles.xml (by default). When Dxfs is fully implemented this can be removed.
    }
    ...
    protected void SetValueColor(XmlHelper helper, string path, ExcelDxfColor color)
    {
        if (color is { HasValue: true })
        {
            ...
            else if (color.Index != null) SetValue(helper, path + "/@indexed", color.Index);
            if (color.Tint != null) SetValue(helper, path + "/@tint", color.Tint);
        }
    }
    ...
    protected void SetValueEnum(XmlHelper helper, string path, Enum v)
    {
        ...
        {
            var s = v.ToString();
            s = s[..1].ToLower(CultureInfo.InvariantCulture) + s[1..];
