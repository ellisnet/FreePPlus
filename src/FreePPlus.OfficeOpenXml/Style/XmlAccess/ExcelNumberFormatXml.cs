...
public int NumFmtId { get; set; }
    ...
    internal ExcelFormatTranslator FormatTranslator
    {
        get
        {
            if (_translator == null) _translator = new ExcelFormatTranslator(Format, NumFmtId);
            return _translator;
        }
    }
    ...
    {
        if (NumFmtId < 0) NumFmtId = ExcelNumberFormat.GetFromBuildIdFromFormat(Format);
        return NumFmtId.ToString();
    }

    internal static void AddBuildIn(XmlNamespaceManager NameSpaceManager,
        ExcelStyleCollection<ExcelNumberFormatXml> NumberFormats)
    {
        ...
        NumberFormats.Add("mm-dd-yy",
            new ExcelNumberFormatXml(NameSpaceManager, true) { NumFmtId = 14, Format = "mm-dd-yy" });
        NumberFormats.Add("d-mmm-yy",
            new ExcelNumberFormatXml(NameSpaceManager, true) { NumFmtId = 15, Format = "d-mmm-yy" });
        NumberFormats.Add("d-mmm",
            new ExcelNumberFormatXml(NameSpaceManager, true) { NumFmtId = 16, Format = "d-mmm" });
        ...
        NumberFormats.Add("h:mm:ss",
            new ExcelNumberFormatXml(NameSpaceManager, true) { NumFmtId = 21, Format = "h:mm:ss" });
        NumberFormats.Add("m/d/yy h:mm",
            new ExcelNumberFormatXml(NameSpaceManager, true) { NumFmtId = 22, Format = "m/d/yy h:mm" });
        NumberFormats.Add("#,##0 ;(#,##0)",
            new ExcelNumberFormatXml(NameSpaceManager, true) { NumFmtId = 37, Format = "#,##0 ;(#,##0)" });
        NumberFormats.Add("#,##0 ;[Red](#,##0)",
            new ExcelNumberFormatXml(NameSpaceManager, true) { NumFmtId = 38, Format = "#,##0 ;[Red](#,##0)" });
        NumberFormats.Add("#,##0.00;(#,##0.00)",
            new ExcelNumberFormatXml(NameSpaceManager, true) { NumFmtId = 39, Format = "#,##0.00;(#,##0.00)" });
        NumberFormats.Add("#,##0.00;[Red](#,##0.00)",
            new ExcelNumberFormatXml(NameSpaceManager, true) { NumFmtId = 40, Format = "#,##0.00;[Red](#,##0.00)" });
        ...
        NumberFormats.Add("mmss.0",
            new ExcelNumberFormatXml(NameSpaceManager, true) { NumFmtId = 47, Format = "mmss.0" });
        NumberFormats.Add("##0.0",
            new ExcelNumberFormatXml(NameSpaceManager, true) { NumFmtId = 48, Format = "##0.0" });
    ...
    internal class ExcelFormatTranslator
    {
        ...
        internal CultureInfo Culture
        {
            get
            {
                if (_ci == null) return CultureInfo.CurrentCulture;
                return _ci;
            }
        ...
        private void ToNetFormat(string ExcelFormat, bool forColWidth)
        {
            ...
            for (var pos = 0; pos < ExcelFormat.Length; pos++)
            {
                ...
                else
                {
                    ...
                    {
                        if (forColWidth) sb.AppendFormat("\"{0}\"", c);
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if (DataType == eFormatType.Unknown)
                            {
                                if (c == '0' || c == '#' || c == '.')
                                    DataType = eFormatType.Number;
                                else if (clc == 'y' || clc == 'm' || clc == 'd' || clc == 'h' || clc == 'm' ||
                                         clc == 's') DataType = eFormatType.DateTime;
                            }
        ...
        internal string FormatFraction(double d)
        {
            ...
            if (!int.TryParse(fmt[1], out fixedDenominator)) fixedDenominator = 0;
            ...
            if (fixedDenominator == 0)
            {
                ...
                var maxNum = 0;
                for (var i = 0; i < maxDigits; i++) maxNum += 9 * (int)Math.Pow(10, i);

                var divRes = 1 / (Math.Abs(d) - intPart);
            ...
            return sign + intPart.ToString(NetFormat)
