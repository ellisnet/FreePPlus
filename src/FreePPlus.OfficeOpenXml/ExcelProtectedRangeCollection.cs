...
public class ExcelProtectedRangeCollection : XmlHelper, IEnumerable<ExcelProtectedRange>
{
        ...
        : base(nsm, topNode)
    {
        ...
        foreach (XmlNode protectedRangeNode in topNode.SelectNodes("d:protectedRanges/d:protectedRange", nsm))
        {
            if (!(protectedRangeNode is XmlElement))
                continue;
            _baseList.Add(new ExcelProtectedRange(protectedRangeNode.Attributes["name"].Value,
                new ExcelAddress(SqRefUtility.FromSqRefAddress(protectedRangeNode.Attributes["sqref"].Value)), nsm,
                topNode));
        }
    }
    ...
    {
        if (!ExistNode("d:protectedRanges")) CreateNode("d:protectedRanges");
    ...
    {
        DeleteAllNode("d:protectedRanges/d:protectedRange[@name='" + item.Name + "' and @sqref='" +
