...
public class ExcelCellAddress
{
    ...
    public int Row
    {
        ...
        private set
        {
            if (value <= 0) throw new ArgumentOutOfRangeException("value", "Row cannot be less than 1.");
            _row = value;
            if (_column > 0)
    ...
    public int Column
    {
        ...
        private set
        {
            if (value <= 0) throw new ArgumentOutOfRangeException("value", "Column cannot be less than 1.");
            _column = value;
            if (_row > 0)
                _address = ExcelCellBase.GetAddress(_row, _column);
            else
    ...
    public static string GetColumnLetter(int column)
    {
        if (column > ExcelPackage.MaxColumns || column < 1)
            throw new InvalidOperationException("Invalid 1-based column index: " + column + ". Valid range is 1 to " +
