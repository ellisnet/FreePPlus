...
public class ExcelRange : ExcelRangeBase
{
    ...
    {
        if (Row < 1 || Row > ExcelPackage.MaxRows) throw new ArgumentException("Row out of range");
        if (Col < 1 || Col > ExcelPackage.MaxColumns) throw new ArgumentException("Column out of range");
    }
