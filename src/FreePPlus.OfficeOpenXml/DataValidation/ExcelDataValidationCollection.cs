...
///     </para>
///     <para>
///         The public methods of this class (Add[...]Validation) will create a datavalidation entry in the worksheet. When
///         this
///         validation has been created changes to the properties will affect the workbook immediately.
///     </para>
///     <para>
///         Each type of validation has either a formula or a typed value/values, except for custom validation which has a
///         formula only.
///     </para>
///     <code>
...
public class ExcelDataValidationCollection : XmlHelper, IEnumerable<IExcelDataValidation>
{
        ...
        : base(worksheet.NameSpaceManager, worksheet.WorksheetXml.DocumentElement)
    {
        ...
        var dataValidationNodes =
            worksheet.WorksheetXml.SelectNodes(DataValidationItemsPath, worksheet.NameSpaceManager);
        ...
        if (_validations.Count > 0) OnValidationCountChanged();
    ...
    private void EnsureRootElementExists()
    {
        ...
        if (node == null) CreateNode(DataValidationPath.TrimStart('/'));
    }
    ...
    private void OnValidationCountChanged()
    {
        ...
        {
            if (dvNode != null) _worksheet.WorksheetXml.DocumentElement.RemoveChild(dvNode);
            _worksheet.ClearValidations();
        }
        else
        {
            var attr = _worksheet.WorksheetXml.DocumentElement.SelectSingleNode(DataValidationPath + "[@count]",
                _worksheet.NameSpaceManager);
            if (attr == null) dvNode.Attributes.Append(_worksheet.WorksheetXml.CreateAttribute("count"));
    ...
    private void ValidateAddress(string address, IExcelDataValidation validatingValidation)
    {
        ...
        if (_validations.Count > 0)
            ...
            {
                if (validatingValidation != null && validatingValidation == validation) continue;
                var result = validation.Address.Collide(newAddress);
                if (result != ExcelAddressBase.eAddressCollition.No)
                    throw new InvalidOperationException(string.Format(
                        "The address ({0}) collides with an existing validation ({1})", address,
    ...
    public bool Remove(IExcelDataValidation item)
    {
        ...
        var dvNode =
            _worksheet.WorksheetXml.DocumentElement.SelectSingleNode(DataValidationPath.TrimStart('/'),
                NameSpaceManager);
