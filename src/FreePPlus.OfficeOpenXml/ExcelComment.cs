...
public class ExcelComment : ExcelVmlDrawingComment
{
        ...
        : base(null, cell, cell.Worksheet.VmlDrawingsComments.NameSpaceManager)
    {
        ...
        if (!cell.Worksheet._vmlDrawings.ContainsKey(ExcelCellBase.GetCellID(cell.Worksheet.SheetID, cell.Start.Row,
                cell.Start.Column))) cell.Worksheet._vmlDrawings.Add(cell);

        TopNode = cell.Worksheet
            .VmlDrawingsComments[ExcelCellBase.GetCellID(cell.Worksheet.SheetID, cell.Start.Row, cell.Start.Column)]
            .TopNode;
        RichText = new ExcelRichTextCollection(ns, textElem);
        var tNode = textElem.SelectSingleNode("d:t", ns);
        if (tNode != null) _text = tNode.InnerText;
    }
    ...
    public string Author
    {
        get
        {
            ...
            return _commentHelper.TopNode.OwnerDocument
                .SelectSingleNode(string.Format("{0}[{1}]", AUTHOR_PATH, authorRef + 1),
                    _commentHelper.NameSpaceManager).InnerText;
        }
        set
        {
    ...
    public ExcelRichText Font
    {
        get
        {
            if (RichText.Count > 0) return RichText[0];
            return null;
        }
    }
    ...
    private int GetAuthor(string value)
    {
        var authorRef = 0;
        var found = false;
        foreach (XmlElement node in _commentHelper.TopNode.OwnerDocument.SelectNodes(AUTHOR_PATH,
        ...
        if (!found)
        {
            ...
            _commentHelper.TopNode.OwnerDocument.SelectSingleNode(AUTHORS_PATH, _commentHelper.NameSpaceManager)
                .AppendChild(elem);
