...
{
    private const string schemaRelVbaSignature =
    ...
    ///         There is no validation that the certificate is valid for codesigning, so make sure it's valid to sign Excel
    ///         files (Excel 2010 is more strict that prior versions).
    ///     </remarks>
    /// </summary>
    ...
    internal void Save(ExcelVbaProject proj)
    {
        ...
        if (Certificate.HasPrivateKey == false) //No signature. Remove any Signature part
        {
            ...
            if (storeCert == null) storeCert = GetCertFromStore(StoreLocation.LocalMachine);
            ...
            {
                foreach (var r in Part.GetRelationships()) Part.DeleteRelationship(r.Id);
        ...
        if (rel == null)
            proj.Part.CreateRelationship(UriHelper.ResolvePartUri(proj.Uri, Uri), TargetMode.Internal,
                schemaRelVbaSignature);
        var b = ms.ToArray();
    ...
    internal byte[] SignProject(ExcelVbaProject proj)
    {
        ...
        bw.Write(new byte[]
            { 0x2B, 0x06, 0x01, 0x04, 0x01, 0x82, 0x37, 0x02, 0x01, 0x1D }); //Encoded Oid 1.3.6.1.4.1.311.2.1.29
        ...
        }); //Encoded Oid for 1.2.840.113549.2.5 (AlgorithmIdentifier MD5)
    ...
    private byte[] GetContentHash(ExcelVbaProject proj)
    {
        ...
        {
            if (reference.ReferenceRecordID == 0x0D) bw.Write((byte)0x7B);
        ...
        {
            var lines = module.Code.Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries);
