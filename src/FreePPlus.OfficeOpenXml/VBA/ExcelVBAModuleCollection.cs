...
public class ExcelVBACollectionBase<T> : IEnumerable<T>
{
    ...
    public T this[string Name]
    {
        ...
        {
            return _list.Find(f =>
                TypeCompat.GetPropertyValue(f, "Name").ToString().Equals(Name, StringComparison.OrdinalIgnoreCase));
        }
    }
...
public class ExcelVbaModuleCollection : ExcelVBACollectionBase<ExcelVBAModule>
{
    ...
    {
        if (this[Name] != null) throw new ArgumentException("Vba modulename already exist.");
    ...
    public ExcelVBAModule AddClass(string Name, bool Exposed)
    {
        ...
        m.Attributes._list.Add(new ExcelVbaModuleAttribute
            { Name = "VB_PredeclaredId", Value = "False", DataType = eAttributeDataType.NonString });
        m.Attributes._list.Add(new ExcelVbaModuleAttribute
        ...
        m.Attributes._list.Add(new ExcelVbaModuleAttribute
            { Name = "VB_Customizable", Value = "False", DataType = eAttributeDataType.NonString });
    ...
    internal string GetAttributeText()
    {
        ...
        foreach (var attr in this)
            sb.AppendFormat("Attribute {0} = {1}\r\n", attr.Name,
                attr.DataType == eAttributeDataType.String ? "\"" + attr.Value + "\"" : attr.Value);
        return sb.ToString();
    }
}