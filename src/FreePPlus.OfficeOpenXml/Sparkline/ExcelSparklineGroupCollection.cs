...
public class ExcelSparklineGroupCollection : IEnumerable<ExcelSparklineGroup>
{
    ...
    ///     The location of the sparkline group. The range must have one row or column and must match
    ///     the number of rows/columns in the datarange
    /// </param>
    ...
    public ExcelSparklineGroup Add(eSparklineType type, ExcelAddressBase locationRange, ExcelAddressBase dataRange)
    {
        if (locationRange.Rows == 1)
        {
            ...
            throw new ArgumentException(
                "dataRange is not valid. dataRange columns or rows must match number of rows in locationRange");
        }
        ...
        if (locationRange.Columns == 1)
        {
            ...
            if (locationRange.Rows == dataRange.Rows)
                return AddGroup(type, locationRange, dataRange, true);
            throw new ArgumentException(
                "dataRange is not valid. dataRange columns or rows must match number of columns in locationRange");
        }

        throw new ArgumentException("locationRange is not valid. Range must be one Column or Row only");
    }

    private ExcelSparklineGroup AddGroup(eSparklineType type, ExcelAddressBase locationRange,
        ExcelAddressBase dataRange, bool isRows)
    {
        ...
        var cells = (locationRange._fromRow == locationRange._toRow
            ? locationRange._toCol - locationRange._fromCol
            : locationRange._toRow - locationRange._fromRow) + 1;
        var cell = 0;
    ...
    {
        var xh = new XmlHelperInstance(_ws.NameSpaceManager,
    ...
    private void LoadSparklines()
    {
        ...
        foreach (XmlElement grp in grps) _lst.Add(new ExcelSparklineGroup(_ws.NameSpaceManager, grp, _ws));
    }
