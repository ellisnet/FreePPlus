...
public abstract class XmlHelper
{
    ...
    {
        // Path is obrigatory
        if (path == null || path == string.Empty) return topNode;
            ...
            if (subPath.Length > 0)
            {
                ...
                if (subPath.StartsWith("@"))
                {
                    ...
                    else
                    {
                        ...
                        if (attributeValue != null) node.Attributes[attributeName].Value = attributeValue;
                    }
                }
                else
                {
    ...
    private int GetNodePos(string nodeName)
    {
        ...
        if (ix > 0) nodeName = nodeName.Substring(ix + 1, nodeName.Length - (ix + 1));
        for (var i = 0; i < SchemaNodeOrder.Length; i++)
    ...
    internal static Uri GetNewUri(ZipPackage package, string sUri, ref int id)
    {
        ...
        while (package.PartExists(uri)) uri = new Uri(string.Format(sUri, ++id), UriKind.Relative);
        return uri;
    }
