...
internal static class ConvertUtil
{
    internal static bool IsNumeric(object candidate)
    {
        ...
        return TypeCompat.IsPrimitive(candidate) || candidate is double || candidate is decimal ||
               candidate is DateTime || candidate is TimeSpan || candidate is long;
    }
    ...
    /// <summary>
    ///     Tries to parse a <see cref="DateTime" /> from the specified <paramref name="candidate" /> which is expected to be a
    ///     string value.
    /// </summary>
    /// <param name="candidate">The string value.</param>
    /// <param name="result">The double value parsed from the specified <paramref name="candidate" />.</param>
    ...
    internal static double GetValueDouble(object v, bool ignoreBool = false, bool retNaN = false)
    {
        ...
        try
        {
            if (ignoreBool && v is bool) return 0;
            if (IsNumeric(v))
            {
                if (v is DateTime)
    ...
    public static T GetTypedCellValue<T>(object value)
    {
        ...
        var toNullableUnderlyingType =
            TypeCompat.IsGenericType(toType) && toType.GetGenericTypeDefinition() == typeof(Nullable<>)
    ...
    internal static CompareInfo
        _invariantCompareInfo =
            CompareInfo.GetCompareInfo(CultureInfo.InvariantCulture.Name); //TODO:Check that it works
