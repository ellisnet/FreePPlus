...
internal static class VBACompression
{
            ...
            for (var i = 0; i < 8; i++)
            {
                if (dPos - startPos > 0)
                {
                    ...
                    while (candidate >= startPos)
                    {
                        if (buffer[candidate] == buffer[dPos])
                        {
                            ...
                            while (buffer.Length > dPos + length &&
                                   buffer[candidate + length] == buffer[dPos + length] && length < lengthMask &&
                                   dPos + length < dEnd) length++;
                            if (length > bestLength)
                            {
                                bestCandidate = candidate;
                                bestLength = length;
                                if (bestLength == lengthMask) break;
                            }
                        }
                    ...
                    if (bestLength >= 3) //Copy token
                    {
                        ...
                        var token = (ushort)(((ushort)(dPos - (bestCandidate + 1)) << bitCount) |
                                             (ushort)(bestLength - 3));
    ...
    {
        if (part[startPos] != 1) return null;
        var ms = new MemoryStream(4096);
        var compressPos = startPos + 1;
        while (compressPos < part.Length - 1) DecompressChunk(ms, part, ref compressPos);
        return ms.ToArray();
    }
    ...
    private static void DecompressChunk(MemoryStream ms, byte[] compBuffer, ref int pos)
    {
        ...
        var size = (header & 0xFFF) + 3;
        var endPos = pos + size;
        var a = (header & 0x7000) >> 12;
        var b = (header & 0x8000) >> 15;
        pos += 2;
            ...
            while (pos < compBuffer.Length && pos < endPos)
            {
                ...
                for (var i = 0; i < 8; i++)
                {
                    ...
                    else //copy token
                    {
                        ...
                        {
                            ms.WriteByte(
                                buffer[source]); //Must copy byte-wise because copytokens can overlap compressed buffer.
