...
internal class CompoundDocumentFile : IDisposable
{
    ...
    private static readonly byte[] header = { 0xD0, 0xCF, 0x11, 0xE0, 0xA1, 0xB1, 0x1A, 0xE1 };
    ...
    private int
        _numberOfFATSectors; // (4 bytes): This integer field contains the count of the number of FAT sectors in the compound file.

    private int
        _firstDirectorySectorLocation; // (4 bytes): This integer field contains the starting sector number for the directory stream.            
    ...
    private int
        _firstMiniFATSectorLocation; // (4 bytes): This integer field contains the starting sector number for the mini FAT. 

    private int
        _numberofMiniFATSectors; // (4 bytes): This integer field contains the count of the number of mini FAT sectors in the compound file. 

    private int
        _firstDIFATSectorLocation; // (4 bytes): This integer field contains the starting sector number for the DIFAT. 

    private int
        _numberofDIFATSectors; // (4 bytes): This integer field contains the count of the number of DIFAT sectors in the compound file. 
    ...
    internal void Read(BinaryReader br)
    {
        ...
        {
            var d = br.ReadInt32();
            if (d >= 0) dwi.DIFAT.Add(d);
        }

        LoadSectors(br);
        if (_firstDIFATSectorLocation > 0) LoadDIFATSectors(dwi);
    ...
    private void LoadSectors(BinaryReader br)
    {
        ...
        while (br.BaseStream.Position < br.BaseStream.Length) _sectors.Add(br.ReadBytes(_sectorSize));
    }
    ...
    private void GetMiniSectors(byte[] miniFATStream)
    {
        ...
        while (br.BaseStream.Position < br.BaseStream.Length) _miniSectors.Add(br.ReadBytes(_miniSectorSize));
    }
    ...
    public void Write(MemoryStream ms)
    {
        ...
        WritePosition(bw, new[] { FAT_SECTOR, END_OF_CHAIN, END_OF_CHAIN },
            ref _currentFATSectorPos); //First sector is first FAT sector, second is First Dir sector, thirs is first Mini FAT sector.
    ...
    private int AddChildren(CompoundDocumentItem item, List<CompoundDocumentItem> l)
    {
        ...
        {
            foreach (var c in item.Children) InitItem(c);

            item.Children.Sort();

            childId = SetSiblings(l.Count, item.Children, 0, item.Children.Count - 1, -1);
            l.AddRange(item.Children);
            foreach (var c in item.Children) c.ChildID = AddChildren(c, l);
        }
    ...
    private void SetUnhandled(int listAdd, List<CompoundDocumentItem> children)
    {
        for (var i = 0; i < children.Count; i++)
            if (children[i]._handled == false)
            {
                ...
                else if (i < children.Count - 1 && children[i + 1].LeftSibling == -1 &&
                         children[i].RightSibling != i + listAdd + 1)
    ...
    private int WriteDirStream(BinaryWriter bw, List<CompoundDocumentItem> dirs)
    {
        if (dirs.Count > 0)
        {
            ...
            for (var i = 0; i < Math.Min(_sectorSize / 128, dirs.Count); i++) dirs[i].Write(bw);
        }
        else
        {
            return -1;
    ...
    private void CheckUpdateDIFAT(BinaryWriter bw)
    {
        if (bw.BaseStream.Position % _sectorSize == 0)
        {
            if (_currentDIFATSectorPos % _sectorSize == 0)
                ...
                (bw.BaseStream.Position == _sectorSize * 2)
            ...
            _numberOfFATSectors++;
            if (_currentDIFATSectorPos == _sectorSize || ((_currentDIFATSectorPos + 4) % _sectorSize == 0 &&
                                                          _currentDIFATSectorPos > _sectorSize))
            {
                ...
                //Fill sector
                for (var i = 0; i < _sectorSize; i++) bw.Write((byte)0xFF);
    ...
    private void AllocateFAT(BinaryWriter bw, int miniFatLength, List<CompoundDocumentItem> dirs)
    {
        ...
        bw.Write(new byte[(numberOfFATSectors + (_numberofDIFATSectors > 0 ? _numberofDIFATSectors - 1 : 0)) *
                          _sectorSize]);
